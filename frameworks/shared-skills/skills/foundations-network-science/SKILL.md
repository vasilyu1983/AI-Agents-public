...
Before reporting a graph statistic, name the target population, observation
window, node/edge inclusion rule, sampling process, and estimand. Centrality and
community results can reverse under missing nodes, projection choices, or edge
weight definitions. Network size alone is never a validity threshold; compare
the result across plausible graph constructions and report unstable ranks.
