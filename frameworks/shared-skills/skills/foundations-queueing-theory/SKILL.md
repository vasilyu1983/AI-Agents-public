...
| 11 | [Fork-Join](assets/templates/queueing-theory/11-fork-join-parallel.md) | E[max]=E[S]·H_K only for iid exponential services; include waiting | Fan-out latency dominated by slowest worker |
...
| **Little's Law applied across non-stationary windows** | L = λW holds only at steady state; burst windows violate ergodicity assumption | Check stationarity and boundary effects across representative windows; analyze transients separately |
...
1. **M/G/1 / P-K (04)**: model the prefill phase with Poisson arrivals and near-deterministic service (prompt-token-proportional duration); compute mean Wq only if the single-server assumptions apply; require ρ < 1 for that stationary model and validate workload-specific headroom against latency and memory targets. No universal .7 degradation threshold applies.
...
- Finite-trace FCFS simulator contract and example: [`references/trace-driven-simulation.md`](references/trace-driven-simulation.md)
...
- **M/M/c finite-time analysis**: stationary Erlang-C is not a universal lower bound on transient capacity. Initial backlog, initial server state, horizon, arrivals and target determine whether stationary sizing over- or underestimates need. An empty M/M/1 at λ=.5, μ=1 starts with zero waiting probability, versus stationary .5. Use a transient model or trace simulation; use any cited finite-time theorem only after checking its exact assumptions and bound against this initial state and target.
...
- Erlang-B and Erlang-C assume Poisson arrivals. Arrival CV²_a > 1 alone does not establish a universal bound on blocking, wait or capacity. Specify arrival dependence and burst structure; compare these stationary baselines against a fitted non-Poisson model or trace simulation.
