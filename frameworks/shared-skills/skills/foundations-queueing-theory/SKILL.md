...
4. **Finite buffer / KV-cache constraint (08)**: treat KV-cache capacity as a shared finite buffer; model admission control analogously to AQM — reject or defer requests when KV-cache occupancy exceeds threshold.
5. **M/M/c autoscaling (03)**: for dynamic replica counts, apply SageServe multi-timescale control (Jaiswal et al., SIGMETRICS 2026) — short-horizon routing + long-horizon GPU scaling via traffic forecasting.
6. **Fleet simulation escalation**: If the token-length distribution is heavy-tailed (measured CV² > 2), combine M/G/c analytical sizing from step 1 with discrete-event simulation (inference-fleet-sim pattern, arXiv 2603.16054) before committing to fleet purchase. Analytical M/G/c alone produces incorrect sizing for split thresholds, GPU type selection, and utilization under heavy-tailed LLM workloads.

_Standout insight_: Standard work-conserving policies (Orca, Sarathi-serve) achieve maximum stable throughput for single-engine LLM serving (Dai, Deng, Li & Peng, SIGMETRICS 2026). The scheduling choice matters most for tail latency, not throughput — apply Trail or Robust Gittins when p99 SLOs are binding.
...
- **M/G/k multiserver scheduling**: The prior assumption that SRPT-k achieves optimal mean response time in M/G/k queues across all loads has been superseded. Grosof & Hurtado-Lange (arXiv 2510.25963, accepted SIGMETRICS 2026) introduce SEK-SMOD, the first policy provably achieving lower mean response time than SRPT-k across all loads and all job size distributions. Treat SRPT-k as a strong but no longer optimal baseline in M/G/k systems.
