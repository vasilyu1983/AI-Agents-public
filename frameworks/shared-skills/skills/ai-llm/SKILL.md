...
- **[Tokenizer Diagnostics](references/tokenizer-diagnostics.md)** - debugging production tokenizer failures: fertility and parity metrics, glitch/undertrained tokens, domain-mismatch fragmentation, number tokenization as an arithmetic failure source, and typo brittleness. Start here when a model misbehaves on only a subset of inputs. The constructive counterpart — *building* a BPE tokenizer — is [ai-pretraining](../ai-pretraining/references/bpe-tokenizer.md).
- **[Dataset Formatting Guide](references/dataset-formatting-guide.md)** - instruction/chat/transformation formats, dataset hygiene, and SFT loss-masking mechanics (`ignore_index=-100`, keeping the first EOS unmasked, per-batch dynamic padding). `ai-pretraining` delegates SFT formatting here.
