...
**Freshness anchor:** Provider-native structured output moves quickly; verify current API docs before depending on feature parity or schema guarantees.

## Table of Contents

- [Provider Feature Comparison](#provider-feature-comparison)
- [Method Selection Decision Tree](#method-selection-decision-tree)
- [OpenAI Structured Outputs](#openai-structured-outputs)
- [Anthropic Tool Use for Structured Output](#anthropic-tool-use-for-structured-output)
- [Gemini Function Declarations](#gemini-function-declarations)
- [Constrained Decoding (Local Models)](#constrained-decoding-local-models)
...
# Pseudocode: adapt exact parameter names to the current SDK version.
...
The table below is a directional illustration of how reliability degrades with schema complexity, not a sourced benchmark for any specific model — treat the numbers as ballpark ordering (frontier strict-mode > frontier best-effort > small local model), and measure your own reliability on your own schemas before shipping.

| Schema Complexity | Frontier tier, strict/guaranteed schema mode | Frontier tier, best-effort (no native strict mode) | Small (~7B) local model |
|---|---|---|---|
| Flat object, <5 fields | Very high | High | Medium-high |
| Nested, <10 fields | Very high | High | Medium |
| Nested, 10-20 fields | High | Medium-high | Low-medium |
| Deeply nested, >20 fields | Medium-high | Medium | Low |
| Array of complex objects | High | Medium-high | Low |
