...
**Freshness anchor:** Treat provider-specific pricing, tokenization formulas, and modality limits in this file as examples that require verification against current official docs before production use.
...
Model picks current as of July 2026; verify against provider docs before committing a stack.

| Task | Best Modality | Recommended Model Class | Cost Tier |
|---|---|---|---|
| Image description | Vision | Balanced/value tier (e.g. current OpenAI Terra/Luna class, Claude Sonnet class) | $$ |
...
> Naming note: avoid pinning specific dated model IDs (e.g. a specific GPT or Claude point release) in this table — provider model families rotate every few months. As of July 2026, current examples include Claude Fable 5 / Opus 4.8 / Sonnet 5 / Haiku 4.5, GPT-5.6 (Luna/Terra/Sol), and Gemini 3.1 Pro / 3.5 Flash — verify current names before quoting one in a design doc.

---

## Vision: Image Token Cost Analysis

### Token Calculation by Provider (Verify Current Docs)

| Provider | Model Class | Pricing Model | Formula |
|---|---|---|---|
| OpenAI | Current flagship/balanced tier | Tiled: per-tile + base (verify current constants) | tiles = ceil(w/512) * ceil(h/512) — confirm formula in current docs |
| OpenAI | Current speed/mini tier | Same tiling, lower $/token | Same formula, cheaper per token |
| Anthropic | Claude Sonnet class | Proportional to image size | ~1600 tokens for 1568x1568 (verify current scaling) |
| Anthropic | Claude Haiku class | Same scaling, lower cost | Same formula, cheaper per token |
| Google | Gemini Flash class | Fixed per-image rate | Flat regardless of size |
| Google | Gemini Pro class | Fixed per-image rate | Flat regardless of size |

> These are illustrative pricing *mechanics* (tiled vs. proportional vs. flat), not current dollar figures. Confirm exact per-tile/per-token constants against each provider's current pricing page before using in a cost model — they change more often than the mechanism itself.
...
### Multimodal Embedding Models (Verify Current Docs)
