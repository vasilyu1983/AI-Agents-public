...
- [Modern Best Practices (2026)](#modern-best-practices-2026)
- [Cost Model Fundamentals](#cost-model-fundamentals)
- [Unit Economics Formula](#unit-economics-formula)
- [Pricing Snapshot Policy (Always Verify)](#pricing-snapshot-policy-always-verify)
- [Prompt Caching Economics (Provider-Specific)](#prompt-caching-economics-provider-specific)
- [Caching ROI calculation](#caching-roi-calculation)
- [Break-even: 2 requests with same prefix](#break-even-2-requests-with-same-prefix)
- [3+ requests: significant savings](#3-requests-significant-savings)
- [Cost-Quality Tradeoff Framework](#cost-quality-tradeoff-framework)
- [The Cost-Quality Curve](#the-cost-quality-curve)
- [Decision Matrix: Model Selection by Use Case](#decision-matrix-model-selection-by-use-case)
- [Tiered Model Strategy (Cascade Pattern)](#tiered-model-strategy-cascade-pattern)
- [Route by complexity to optimize cost/quality](#route-by-complexity-to-optimize-costquality)
- [Expected savings: 40-60% vs always using top model](#expected-savings-40-60%-vs-always-using-top-model)
- [Quality-Cost Scoring Template](#quality-cost-scoring-template)
- [Total Cost of Ownership (TCO)](#total-cost-of-ownership-tco)
- [LLM TCO Components](#llm-tco-components)
- [TCO Calculation Template](#tco-calculation-template)
- [Self-Hosted vs API TCO](#self-hosted-vs-api-tco)
- [Fine-Tuning ROI Framework](#fine-tuning-roi-framework)
- [When Fine-Tuning Pays Off](#when-fine-tuning-pays-off)
- [Fine-Tuning Cost Components](#fine-tuning-cost-components)
- [Fine-Tuning Decision Matrix](#fine-tuning-decision-matrix)
- [Fine-Tuning Break-Even Analysis](#fine-tuning-break-even-analysis)
- [Calculate break-even point for fine-tuning investment](#calculate-break-even-point-for-fine-tuning-investment)
- [Example: $50k fine-tuning, 100k req/month](#example-$50k-fine-tuning-100k-reqmonth)
- [Current: $0.05/req, Tuned (smaller model): $0.01/req](#current-$005req-tuned-smaller-model-$001req)
- [Savings: $4k/month → Break-even: 12.5 months](#savings-$4kmonth-→-break-even-125-months)
- [Fine-Tuning vs Prompting Cost Comparison](#fine-tuning-vs-prompting-cost-comparison)
- [Prompt Optimization for Cost](#prompt-optimization-for-cost)
- [Token Reduction Techniques](#token-reduction-techniques)
- [Prompt Cost Audit Checklist](#prompt-cost-audit-checklist)
- [Example: Prompt Optimization](#example-prompt-optimization)
- [Cost Monitoring & Alerting](#cost-monitoring-&-alerting)
- [Essential Metrics](#essential-metrics)
- [Cost Dashboard Components](#cost-dashboard-components)
- [Budget Guardrails](#budget-guardrails)
- [Implement hard limits to prevent cost runaway](#implement-hard-limits-to-prevent-cost-runaway)
...
- [Strategic (Quarter 1)](#strategic-quarter-1)
...
- [Anthropic Models Overview](https://platform.claude.com/docs/en/about-claude/models/overview) - Current Claude pricing and specs (always verify before planning)
- [OpenAI Pricing](https://openai.com/api/pricing/) - Current model pricing
- [Google Gemini API Models](https://ai.google.dev/gemini-api/docs/models) - Current Gemini pricing and model list
...
**July 2026 price anchors** (verify against official sources before use — prices change and introductory rates expire):

| Model | Input (per MTok) | Output (per MTok) | Notes |
|-------|-----------------|-------------------|-------|
| Claude Fable 5 | $10 | $50 | GA since 2026-06-09; safety-classifier fallback to Opus 4.8 in <5% of sessions |
...
>
> Introductory-pricing trap: Sonnet 5's $2/$10 rate is time-boxed to 2026-08-31. A cost model built during the promo window and left unreviewed will silently understate spend by ~50% once list price ($3/$15) takes effect — pin a review date in the cost dashboard, not just the price.
