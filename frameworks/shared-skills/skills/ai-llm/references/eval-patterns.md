...
- [Traceability Requirements](#traceability-requirements-2026-standard)
- [LLM-as-Judge Best Practices](#llm-as-judge-best-practices-2026)
...
### Offline Benchmark Evaluation: lm-eval-harness

**lm-evaluation-harness** (EleutherAI) is the de facto open standard for offline benchmark evaluation of language models. Use it when comparing base models, fine-tuned checkpoints, or quantized variants against standard academic benchmarks.

- **Repo**: `EleutherAI/lm-evaluation-harness` (verify current version at https://github.com/EleutherAI/lm-evaluation-harness)
- **Covered tasks**: MMLU, HellaSwag, ARC, TruthfulQA, GSM8K, HumanEval, BIG-Bench subsets, and hundreds more
- **Integrations**: Hugging Face Hub models, vLLM backends, OpenAI API-compatible endpoints
- **Best practices**:
  - Always pin the harness version used alongside the checkpoint hash — results drift across harness versions
  - Report few-shot count (0-shot vs 5-shot) and normalization strategy; these affect scores significantly
  - Do not compare numbers across different harness versions without re-running
  - Contamination scan your training corpus against the benchmark task data; contaminated eval scores are not meaningful
  - For production model selection, supplement benchmark numbers with task-specific golden-set evals — lm-eval benchmarks measure general capability, not product-specific quality

| When to use lm-eval-harness | When NOT to use it |
|-----------------------------|-------------------|
| Comparing checkpoint quality across training runs | Measuring end-to-end product quality |
| Validating that fine-tuning did not degrade base capability | As the only eval gate for production release |
| Reporting standard numbers in papers or model cards | When task is not covered by any benchmark task |
| Catching capability regressions during mid-training | When you need real-user quality signals |
...
|-----------|----------|-------|
| **lm-evaluation-harness** | Offline benchmark comparison across checkpoints | De facto open standard; pin version |
