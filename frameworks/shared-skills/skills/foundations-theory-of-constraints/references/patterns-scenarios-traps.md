...
| Evidence | Queue depth and wait time persist across windows, nominating a candidate; changing its effective capacity or policy changes accepted end-to-end throughput at fixed quality | A single snapshot of utilization or a single bad sprint |
...
**Diagnostic sequence**: use several representative windows to reject one-off spikes and nominate persistent candidates. Then vary a candidate's effective capacity or policy through a controlled change, replay, or natural experiment and measure accepted end-to-end throughput at a fixed quality threshold. Persistence without throughput sensitivity is not enough to name the constraint.
...
1. Ask "why is this step slow?" repeatedly (see primitive #5, CRT) to compare plausible resource, demand/market, policy, quality and dependency causes; stop at testable evidence rather than forcing a policy answer.
...
3. If elevating physical capacity has already been tried and throughput did not move, revisit intervention effectiveness, quality/mix, demand, shared resources and shifted constraints; a policy cause is one hypothesis, not an implication — see `10-policy-constraints.md`.

## When the Single-Constraint Assumption Breaks

TOC's core simplifying assumption — one dominant constraint gates the chosen goal during the observation window — is a modeling choice, not a law of nature. It breaks down in identifiable, common situations. Naming these explicitly is what separates expert application from mechanical checklist use:

| Situation | Why Single-Constraint TOC Misfires | What To Do Instead |
|---|---|---|
| Multiple roughly equal capacity constraints (whose capacity estimates are near-tied within their uncertainty) | Elevating "the" constraint just promotes the next one within one cycle; ranking noise dominates the T/CU signal | Use queueing-network analysis (`foundations-queueing-theory`) to model joint capacity, or treat the near-tied resources as a combined constraint pool |
| Matrix / shared-resource organizations | The same senior engineer, approver, or specialist is claimed as "the constraint" by every team simultaneously; there is no single system, only overlapping systems competing for one resource | Model it explicitly as a shared-resource contention problem: model all competing initiatives and shared capacities together. T/CU ordering is optimal only for divisible independent work against one linear capacity with compatible demand; indivisibility, dependencies, deadlines or multiple capacities require a global feasibility/optimization model |
