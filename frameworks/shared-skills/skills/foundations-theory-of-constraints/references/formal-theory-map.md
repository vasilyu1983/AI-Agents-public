...
When multiple interacting UDEs resist single-root CRT analysis — particularly in complex multi-stakeholder environments — combine CRT with qualitative system dynamics (Qual SD) modeling. Mabin & Cavana (System Dynamics Review, Wiley, 2024; DOI: 10.1002/sdr.1768) demonstrate that the combination produces more insights than either method alone via a six-stage integration framework. Apply when: (1) more than three UDEs connect through feedback loops rather than simple chains, or (2) stakeholders disagree on cause arrows because the system has genuine circular causality. Note: primary paper is behind a paywall; volume/issue could not be independently confirmed.

## Relationship to Kanban and DevOps Flow (as of July 2026)
