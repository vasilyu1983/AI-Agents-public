...
**Worked example:** 8-stage agent pipeline — search → plan → tool-dispatch → eval → summarize. Profiling shows decode at the plan stage = 68% of wall-clock. Exploitation: cache the system prompt prefix (KV reuse); route simple planning decisions to a smaller model. After exploitation, throughput doubles; the constraint shifts to the eval/guardrail stage. Re-run 5FS. Do not add GPUs before verifying exploitation is exhausted.
...
- CCPM evidence base: a 2025 systematic review (MDPI Applied Sciences, DOI: 10.3390/app15158147) analyzed 62 CCPM studies — the strongest available peer-reviewed evidence for this primitive. See `09-critical-chain.md`.
- AI-era constraint shift (dev workflow): reported observation (not controlled-study fact) — AI coding adoption moves the constraint from writing code to reviewing it. Named sources: IT Revolution (Jan 2026), Logilica (Dec 2025), Faros AI 2025 telemetry (10,000+ developers). Re-run 5FS after significant AI tool adoption.
- AI-era constraint shift (runtime / agent pipelines): in multi-step LLM and agent pipelines, the constraint is typically not raw model capacity but system-level stages — decode latency of Planner/Arbiter LLM components (~70% of wall-clock in studied systems), KV-cache management, concurrent session handling, or downstream guardrail/eval stages. Evidence grade B: practitioner and systems-research analysis (2025–2026). Exploit (KV cache reuse, batching, model right-sizing) before adding GPU capacity.
