...
- [Playwright v1.57+ Features](#playwright-v157-features)
...
- [Updated Docker Images](#updated-docker-images)
- [GitLab CI](#gitlab-ci)
- [GitHub Actions](#github-actions)
...
The image tag below must match the `@playwright/test` version pinned in `package.json` — a mismatch (e.g. running `v1.61.1-noble` with `@playwright/test@1.58.x` installed) causes silent browser/driver protocol mismatches. As of 2026-07-11 the latest tag is `v1.61.1-noble`; verify the current tag at https://mcr.microsoft.com/en-us/artifact/mar/playwright/tags before pinning.
...
export default defineConfig({
  ...
  // Do NOT pin workers=1 in CI — it serializes the suite and defeats the
  // sharded matrix above. Let Playwright pick (CPU-based) and rely on
  // `--shard=N/M` for parallelism across runners. Override only if the suite
  // truly cannot run in parallel (legacy shared state).
...
As of 2026-07-11, the current stable release is **1.61.1** (1.61.0 shipped 2026-06-15, patched 2026-06-23). Verify against https://playwright.dev/docs/release-notes before pinning a version in CI images or Docker tags — this table will drift.

| Version | Release | Key CI-relevant changes |
|---------|---------|-------------------------|
| 1.61 (current) | June 2026 | Native passkey/virtual-authenticator testing; first-class WebStorage API (no more `page.evaluate` boilerplate for localStorage); network parity (`securityDetails()`/`serverAddr()` on API responses); new `retain-on-failure-and-retries` video mode; `testInfo.errors` splits `AggregateError` into separate entries. No breaking changes in this release. |
| 1.60 | May 2026 | `tracing.startHar()` / `tracing.stopHar()`; `locator.drop()` for file drag-and-drop; `test.abort()` for fixture/hook-level test termination; page-level `toMatchAriaSnapshot()`; `boxes` option on `ariaSnapshot()`; removed `Locator.ariaRef()`, `videosPath`, `videoSize` |
| 1.59 | April 2026 | `page.screencast` API for agentic video receipts; `browser.bind()` for shared browser sessions; `--debug=cli` agent debugger; `npx playwright trace` CLI analysis; `await using` async disposables |
| 1.57 | ~Feb 2026 | Speedboard HTML reporter tab (slowest-tests view); switch to Chrome-for-Testing builds (`chrome` headed / `chrome-headless-shell` headless) |

**Breaking changes to watch:**
- v1.61: none (first release in this run with zero deprecations, per release notes)
- v1.60: `Locator.ariaRef()` removed; `videosPath`/`videoSize` context options removed
- v1.59: macOS 14 WebKit support removed; `@playwright/experimental-ct-svelte` discontinued (last published at 1.58.2 — migrate Svelte component tests to the main `@playwright/test` runner or pin to 1.58)
- v1.57: `_react` and `_vue` selectors removed; `devtools` option in `browserType.launch()` deprecated
...
1) Copy `assets/template-playwright-fail-on-flaky-reporter.js` into your repo only if you are pinned to an older Playwright version.
