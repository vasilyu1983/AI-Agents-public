...
- [Pattern 1: bcrypt (Recommended)](#pattern-1-bcrypt-recommended)
- [Pattern 2: Argon2 (Best for New Projects)](#pattern-2-argon2-best-for-new-projects)
- [Pattern 3: scrypt](#pattern-3-scrypt)
- [Symmetric Encryption](#symmetric-encryption)
- [Pattern 1: AES-256-GCM (Recommended)](#pattern-1-aes-256-gcm-recommended)
- [Pattern 2: AES-256-CBC (Legacy Support)](#pattern-2-aes-256-cbc-legacy-support)
- [Asymmetric Encryption](#asymmetric-encryption)
- [Pattern 1: RSA](#pattern-1-rsa)
- [Pattern 2: Elliptic Curve (ECDH)](#pattern-2-elliptic-curve-ecdh)
- [Digital Signatures](#digital-signatures)
- [Pattern 1: RSA-PSS](#pattern-1-rsa-pss)
- [Pattern 2: ECDSA](#pattern-2-ecdsa)
- [Hashing](#hashing)
- [Pattern 1: SHA-256 (Data Integrity)](#pattern-1-sha-256-data-integrity)
- [Random Number Generation](#random-number-generation)
- [Pattern 1: Cryptographically Secure Random](#pattern-1-cryptographically-secure-random)
- [TLS/SSL Configuration](#tlsssl-configuration)
- [Pattern 1: Node.js HTTPS Server](#pattern-1-nodejs-https-server)
- [Pattern 2: Express Security Headers](#pattern-2-express-security-headers)
- [Key Management](#key-management)
- [Pattern 1: Environment Variables](#pattern-1-environment-variables)
- [Pattern 2: Key Management Service](#pattern-2-key-management-service)
- [Pattern 3: HashiCorp Vault](#pattern-3-hashicorp-vault)
...
app.use(helmet({
  ...
  // X-XSS-Protection / Expect-CT deprecated — use CSP frame-ancestors / Content-Security-Policy
}));
```
...
NIST finalized its first three post-quantum cryptography (PQC) standards on August 13, 2024:

- **FIPS 203 (ML-KEM)** — module-lattice-based key encapsulation, derived from CRYSTALS-Kyber. Use for key exchange.
- **FIPS 204 (ML-DSA)** — module-lattice-based digital signatures, derived from CRYSTALS-Dilithium. Primary signature standard.
- **FIPS 205 (SLH-DSA)** — stateless hash-based signatures, derived from SPHINCS+. Backup signature standard with different security assumptions than ML-DSA.

A fourth standard, **FIPS 206 (FN-DSA, based on FALCON)**, is in progress for compact signatures but was not yet final as of this writing — verify current status before depending on it.

**When this matters now, not just "eventually":**
- "Harvest now, decrypt later" — any data encrypted today with classical-only algorithms (RSA, ECDH) that must stay confidential beyond roughly a 10-15 year horizon is already at risk from adversaries capturing ciphertext for future decryption once cryptographically relevant quantum computers exist. This is a present-day design input for long-lived secrets (medical records, national-security data, some financial and identity records), not a future-only concern.
- Regulated and high-assurance environments (US federal systems under CNSA 2.0, and vendors selling into them) are on published migration timelines with target completion windows in the early-to-mid 2030s for many categories — check current agency guidance rather than assuming any specific date.
- Practical near-term posture for most applications: adopt **hybrid key exchange** (classical ECDH combined with ML-KEM) where TLS libraries and CDNs support it, so the design degrades gracefully rather than requiring a disruptive cutover. Do not replace classical signatures wholesale yet — ML-DSA/SLH-DSA tooling, library support, and interoperability are still maturing across ecosystems as of mid-2026.
- Symmetric primitives (AES-256, SHA-256/384) are considered quantum-resistant with adequate key/output length (AES-256 rather than AES-128) and do not need replacement; the urgent exposure is in asymmetric key exchange and signatures.

**Action for this skill's default guidance:** treat PQC as a roadmap item to flag during threat modeling for systems with long confidentiality horizons or regulatory PQC mandates, not as a required default for every new TLS or signing implementation today. Re-verify NIST, CNSA 2.0, and browser/CDN hybrid-KEM support before committing to a migration timeline — this is one of the fastest-moving areas in applied cryptography.
...
- [NIST FIPS 203/204/205 (Post-Quantum Cryptography Standards)](https://csrc.nist.gov/pubs/fips/203/final)
