# Threat Modeling Guide — Methodology and Practice
...
- [When to Threat Model](#when-to-threat-model)
- [When NOT to Threat Model](#when-not-to-threat-model)
- [STRIDE Methodology](#stride-methodology)
- [STRIDE Categories](#stride-categories)
- [Applying STRIDE](#applying-stride)
...
- [When to Use PASTA vs STRIDE](#when-to-use-pasta-vs-stride)
- [Data Flow Diagrams (DFDs)](#data-flow-diagrams-dfds)
- [DFD Elements](#dfd-elements)
- [Example: Web Application DFD](#example-web-application-dfd)
- [Trust Boundaries to Identify](#trust-boundaries-to-identify)
- [Attack Tree Construction](#attack-tree-construction)
- [Structure](#structure)
- [Building Effective Attack Trees](#building-effective-attack-trees)
- [Threat Modeling for Microservices](#threat-modeling-for-microservices)
- [Microservices-Specific Threats](#microservices-specific-threats)
- [Microservices DFD Pattern](#microservices-dfd-pattern)
- [Threat Modeling for Web Applications](#threat-modeling-for-web-applications)
- [Standard Web App DFD Components](#standard-web-app-dfd-components)
- [Risk Scoring](#risk-scoring)
- [CVSS (Common Vulnerability Scoring System)](#cvss-common-vulnerability-scoring-system)
- [DREAD (Simplified Risk Model)](#dread-simplified-risk-model)
- [Custom Risk Matrix](#custom-risk-matrix)
- [Tooling](#tooling)
- [Tool Selection Decision](#tool-selection-decision)
- [Lightweight Threat Modeling for Agile Teams](#lightweight-threat-modeling-for-agile-teams)
- [The 15-Minute Threat Model](#the-15-minute-threat-model)
- [Security Story Template](#security-story-template)
- [Anti-Patterns](#anti-patterns)
- [Before Finalizing: Confirmation Gate and Pre-Finalization Checklist](#before-finalizing-confirmation-gate-and-pre-finalization-checklist)
...
CVSS 4.0 is the newest standard (2023); v3.1 still widely deployed.
...
## Before Finalizing: Confirmation Gate and Pre-Finalization Checklist

The methodology sections above cover *what* to model. This section covers a *stop condition*: a point in the workflow where the model must not proceed straight to a final report without a human checkpoint, and a checklist gate that runs immediately before the deliverable ships.

### Mandatory Pause for Confirmation

Before producing the final threat model report, stop and get explicit input on any assumption that materially changes the priority ranking — do not run the full workflow end-to-end and present a finished report as the first thing the requester sees.

1. Summarize the assumptions made so far that would shift a threat's risk score if wrong (examples: is this service internet-facing or internal-only? single-tenant or multi-tenant? what does the deployment model look like? who owns the service?).
2. Ask 1–3 targeted questions to close the gaps that matter most — favor the questions whose answers flip a threat's disposition (e.g., internet exposure turning a "Low" DoS risk into "Critical").
3. Wait for a response before finalizing. If the requester does not answer, do not silently pick an assumption and move on — state explicitly which assumptions remain open and how the ranking would change under each plausible answer.

This is distinct from the review checklist below: the confirmation pause resolves missing *context*; the checklist below verifies *completeness* of what was already modeled.

### 8-Item Pre-Finalization Checklist

Run this immediately before the final report is produced — not as a nice-to-have retrospective, but as a hard gate:

1. Every discovered entry point is represented in the model (no silently dropped attack surface).
2. Every identified trust boundary has at least one corresponding threat entry.
3. Runtime threats are kept separate from CI/dev-pipeline threats — a build-time risk and a production risk are not the same finding.
4. User clarifications from the confirmation pause (or explicit non-responses) are reflected in the final ranking, not just collected and discarded.
5. Remaining assumptions and open questions are stated explicitly in the output, not buried or omitted.
6. The output format matches whatever contract was requested (e.g., a specific report template, ticket format, or the STRIDE table above) — a technically correct model in the wrong shape still fails the deliverable.
7. Each threat has a disposition (Accept / Mitigate / Transfer / Avoid) and, where mitigated, an owner.
8. Risk scores use the methodology declared at the start of the session (CVSS, DREAD, or the custom matrix) — don't silently switch scoring systems mid-model.

Item 7 overlaps with the Threat Model Review Checklist below; both gates should pass together before the report ships.
...
- The confirmation-gate and pre-finalization checklist pattern in [Before Finalizing](#before-finalizing-confirmation-gate-and-pre-finalization-checklist) is adapted from `security-threat-model` (author: openai) in [davila7/claude-code-templates](https://github.com/davila7/claude-code-templates) at commit `22d8efa9e9afcf31b98b7e3952ec557694e72c13`, MIT licensed. Extracted 2026-08-09.
