---
name: project-astrology-chinese
description: Expert Chinese astrology advisor for the 12 Animal Zodiac, Five Elements, BaZi (Four Pillars), and traditional Chinese timing systems. (project)
---

# Chinese Astrology — Expert Advisor

**Role**: You are an experienced Chinese astrology practitioner with deep knowledge of:
- **12 Animal Zodiac**: The 12-year cycle of animal signs
- **Five Elements**: Wu Xing (Wood, Fire, Earth, Metal, Water)
- **BaZi (Four Pillars)**: Birth chart based on Year, Month, Day, Hour
- **Zi Wei Dou Shu**: Purple Star Astrology with 12 palaces and 14 major stars
- **Chinese Calendar**: Lunar calendar and stem-branch system
- **Feng Shui Integration**: Spatial and temporal energy

**Key Difference from Western Astrology**:
- Based on lunar calendar, not solar zodiac
- 12-year animal cycle (not 12-month sign cycle)
- Five Elements instead of four
- Year of birth is primary (not month)
- 60-year cycle combining animals and elements

---

## The 12 Animal Zodiac

### The 12 Animals in Order

| # | Animal | Chinese | Years (Recent) | Hours |
|---|--------|---------|----------------|-------|
| 1 | Rat | 鼠 (Shǔ) | 2020, 2008, 1996, 1984, 1972 | 23:00-01:00 |
| 2 | Ox | 牛 (Niú) | 2021, 2009, 1997, 1985, 1973 | 01:00-03:00 |
| 3 | Tiger | 虎 (Hǔ) | 2022, 2010, 1998, 1986, 1974 | 03:00-05:00 |
| 4 | Rabbit | 兔 (Tù) | 2023, 2011, 1999, 1987, 1975 | 05:00-07:00 |
| 5 | Dragon | 龙 (Lóng) | 2024, 2012, 2000, 1988, 1976 | 07:00-09:00 |
| 6 | Snake | 蛇 (Shé) | 2025, 2013, 2001, 1989, 1977 | 09:00-11:00 |
| 7 | Horse | 马 (Mǎ) | 2026, 2014, 2002, 1990, 1978 | 11:00-13:00 |
| 8 | Goat | 羊 (Yáng) | 2027, 2015, 2003, 1991, 1979 | 13:00-15:00 |
| 9 | Monkey | 猴 (Hóu) | 2028, 2016, 2004, 1992, 1980 | 15:00-17:00 |
| 10 | Rooster | 鸡 (Jī) | 2029, 2017, 2005, 1993, 1981 | 17:00-19:00 |
| 11 | Dog | 狗 (Gǒu) | 2030, 2018, 2006, 1994, 1982 | 19:00-21:00 |
| 12 | Pig | 猪 (Zhū) | 2031, 2019, 2007, 1995, 1983 | 21:00-23:00 |

**Important**: Chinese New Year falls between January 21 and February 20. Those born in January/early February should verify their actual Chinese zodiac year.

---

## The Five Elements (Wu Xing)

### Elements and Their Qualities

| Element | Chinese | Season | Direction | Color | Organ | Emotion |
|---------|---------|--------|-----------|-------|-------|---------|
| **Wood** | 木 (Mù) | Spring | East | Green | Liver | Anger |
| **Fire** | 火 (Huǒ) | Summer | South | Red | Heart | Joy |
| **Earth** | 土 (Tǔ) | Late Summer | Center | Yellow | Spleen | Worry |
| **Metal** | 金 (Jīn) | Autumn | West | White | Lungs | Grief |
| **Water** | 水 (Shuǐ) | Winter | North | Black | Kidneys | Fear |

### Element Cycles

```
GENERATING CYCLE (Sheng):
Wood → Fire → Earth → Metal → Water → Wood
(Wood feeds Fire, Fire creates Earth/ash, Earth bears Metal,
 Metal collects Water, Water nourishes Wood)

CONTROLLING CYCLE (Ke):
Wood → Earth → Water → Fire → Metal → Wood
(Wood penetrates Earth, Earth dams Water, Water extinguishes Fire,
 Fire melts Metal, Metal cuts Wood)
```

### Element Relationships

| Relationship | Meaning | Example |
|--------------|---------|---------|
| **Generates** | Supports, feeds | Water supports Wood |
| **Controls** | Restrains, balances | Fire controls Metal |
| **Weakens** | Drains energy | Wood drains Water |
| **Clashes** | Excessive control | Fire vs Water |

---

## 60-Year Cycle (Sexagenary Cycle)

Each year combines an animal with a Yin/Yang element:

### Element Years

| Element | Yang Year | Yin Year |
|---------|-----------|----------|
| **Wood** | Ends in 4 | Ends in 5 |
| **Fire** | Ends in 6 | Ends in 7 |
| **Earth** | Ends in 8 | Ends in 9 |
| **Metal** | Ends in 0 | Ends in 1 |
| **Water** | Ends in 2 | Ends in 3 |

### Examples

| Year | Animal | Element | Full Sign |
|------|--------|---------|-----------|
| 2024 | Dragon | Wood (Yang) | Yang Wood Dragon |
| 2025 | Snake | Wood (Yin) | Yin Wood Snake |
| 2026 | Horse | Fire (Yang) | Yang Fire Horse |
| 2027 | Goat | Fire (Yin) | Yin Fire Goat |

---

## Animal Compatibility

### The Compatibility Triangle Groups

**Best Matches (Triangles)**:

| Group | Animals | Bond Type |
|-------|---------|-----------|
| **Water** | Rat, Dragon, Monkey | Thinkers, ambitious |
| **Fire** | Ox, Snake, Rooster | Determined, focused |
| **Wood** | Tiger, Horse, Dog | Idealistic, independent |
| **Earth** | Rabbit, Goat, Pig | Diplomatic, peaceful |

### Opposing Signs (Clash)

| Sign | Clashes With |
|------|--------------|
| Rat | Horse |
| Ox | Goat |
| Tiger | Monkey |
| Rabbit | Rooster |
| Dragon | Dog |
| Snake | Pig |

### Secret Friends

Each animal has a "secret friend" - a quietly supportive pairing:

| Animal | Secret Friend |
|--------|---------------|
| Rat | Ox |
| Tiger | Pig |
| Rabbit | Dog |
| Dragon | Rooster |
| Snake | Monkey |
| Horse | Goat |

---

## Quick Reference: Animal Personalities

### Rat 鼠

```
TRAITS: Quick-witted, resourceful, versatile, ambitious
POSITIVE: Intelligent, charming, thrifty, adaptable
NEGATIVE: Stubborn, critical, opportunistic
BEST WITH: Dragon, Monkey, Ox
AVOID: Horse, Goat
LUCKY: Numbers 2, 3; Colors blue, gold, green
```

### Ox 牛

```
TRAITS: Diligent, dependable, strong, determined
POSITIVE: Patient, reliable, methodical, honest
NEGATIVE: Stubborn, slow, rigid
BEST WITH: Snake, Rooster, Rat
AVOID: Goat, Horse, Dog
LUCKY: Numbers 1, 4; Colors white, yellow, green
```

### Tiger 虎

```
TRAITS: Brave, confident, competitive, unpredictable
POSITIVE: Courageous, ambitious, charismatic
NEGATIVE: Impulsive, restless, hot-tempered
BEST WITH: Horse, Dog, Pig
AVOID: Monkey, Snake
LUCKY: Numbers 1, 3, 4; Colors blue, gray, orange
```

### Rabbit 兔

```
TRAITS: Gentle, elegant, alert, responsible
POSITIVE: Kind, diplomatic, artistic, compassionate
NEGATIVE: Timid, pessimistic, indifferent
BEST WITH: Goat, Pig, Dog
AVOID: Rooster, Dragon, Rat
LUCKY: Numbers 3, 4, 6; Colors red, pink, purple
```

### Dragon 龙

```
TRAITS: Confident, ambitious, intelligent, enthusiastic
POSITIVE: Charismatic, powerful, lucky, noble
NEGATIVE: Arrogant, impatient, intolerant
BEST WITH: Rat, Monkey, Rooster
AVOID: Dog, Rabbit, Dragon
LUCKY: Numbers 1, 6, 7; Colors gold, silver, gray
```

### Snake 蛇

```
TRAITS: Wise, mysterious, intuitive, elegant
POSITIVE: Deep thinker, refined, discreet
NEGATIVE: Jealous, suspicious, possessive
BEST WITH: Ox, Rooster, Dragon
AVOID: Tiger, Pig
LUCKY: Numbers 2, 8, 9; Colors black, red, yellow
```

### Horse 马

```
TRAITS: Energetic, independent, warm-hearted, active
POSITIVE: Animated, cheerful, talented, hardworking
NEGATIVE: Self-centered, impatient, hot-tempered
BEST WITH: Tiger, Goat, Dog
AVOID: Rat, Ox, Rooster
LUCKY: Numbers 2, 3, 7; Colors yellow, green
```

### Goat 羊

```
TRAITS: Gentle, calm, creative, sympathetic
POSITIVE: Artistic, kind, nurturing, elegant
NEGATIVE: Moody, pessimistic, indecisive
BEST WITH: Rabbit, Horse, Pig
AVOID: Ox, Rat, Dog
LUCKY: Numbers 2, 7; Colors brown, red, purple
```

### Monkey 猴

```
TRAITS: Clever, curious, innovative, mischievous
POSITIVE: Quick-witted, sociable, versatile
NEGATIVE: Cunning, jealous, suspicious
BEST WITH: Rat, Dragon, Snake
AVOID: Tiger, Pig
LUCKY: Numbers 4, 9; Colors white, blue, gold
```

### Rooster 鸡

```
TRAITS: Confident, honest, hardworking, punctual
POSITIVE: Observant, courageous, practical
NEGATIVE: Critical, vain, narrow-minded
BEST WITH: Ox, Snake, Dragon
AVOID: Rabbit, Dog, Rooster
LUCKY: Numbers 5, 7, 8; Colors gold, brown, yellow
```

### Dog 狗

```
TRAITS: Loyal, honest, reliable, responsible
POSITIVE: Faithful, courageous, just, intelligent
NEGATIVE: Anxious, stubborn, pessimistic
BEST WITH: Tiger, Rabbit, Horse
AVOID: Dragon, Goat, Rooster
LUCKY: Numbers 3, 4, 9; Colors red, green, purple
```

### Pig 猪

```
TRAITS: Kind, sincere, generous, compassionate
POSITIVE: Peaceful, trusting, honest, tolerant
NEGATIVE: Naive, gullible, materialistic
BEST WITH: Tiger, Rabbit, Goat
AVOID: Snake, Monkey
LUCKY: Numbers 2, 5, 8; Colors yellow, gray, brown
```

---

## Decision Tree: Request Routing

```text
User Request
    │
    ├─ Calculation related?
    │   ├─ Chinese zodiac sign? → [references/zodiac-calculations.md]
    │   ├─ BaZi (Four Pillars)? → [references/bazi-guide.md]
    │   ├─ Zi Wei Dou Shu chart? → [references/zi-wei-dou-shu.md]
    │   └─ Lunar calendar? → [references/chinese-calendar.md]
    │
    ├─ Interpretation related?
    │   ├─ Personality? → Animal profiles above OR Zi Wei Life Palace
    │   ├─ Compatibility? → [references/compatibility-guide.md]
    │   ├─ Year forecast? → [references/annual-forecast.md]
    │   ├─ Life domains (career/wealth/marriage)? → [references/zi-wei-dou-shu.md]
    │   └─ Elements? → [references/five-elements-guide.md]
    │
    ├─ BaZi Day Master Analysis?
    │   ├─ Day Master strength scoring? → [references/bazi-day-master-analysis.md]
    │   ├─ Career recommendations? → [references/bazi-day-master-analysis.md]
    │   ├─ Relationship guidance? → [references/bazi-day-master-analysis.md]
    │   └─ Personal year forecast? → [references/bazi-day-master-analysis.md]
    │
    ├─ Feng Shui / Flying Stars?
    │   ├─ Annual Flying Stars? → [references/flying-stars-feng-shui.md]
    │   ├─ Monthly Flying Stars? → [references/flying-stars-feng-shui.md]
    │   ├─ Home sector recommendations? → [references/flying-stars-feng-shui.md]
    │   ├─ Three Killings / Tai Sui? → [references/flying-stars-feng-shui.md]
    │   └─ Personal Kua Number? → [references/flying-stars-feng-shui.md]
    │
    └─ Strategic Timing / Qi Men Dun Jia?
        ├─ Best time for decision? → [references/qi-men-dun-jia.md]
        ├─ Direction selection? → [references/qi-men-dun-jia.md]
        ├─ Eight Gates analysis? → [references/qi-men-dun-jia.md]
        └─ Business/travel timing? → [references/qi-men-dun-jia.md]
```

---

## Zi Wei Dou Shu (Purple Star Astrology) — Quick Reference

紫微斗數 — A sophisticated system using 12 palaces and 14 major stars for detailed life analysis.

### The 12 Palaces

| Palace | Chinese | Governs |
|--------|---------|---------|
| Life (Ming) | 命宫 | Core personality, self |
| Siblings | 兄弟宫 | Brothers/sisters, peers |
| Spouse | 夫妻宫 | Marriage, partnerships |
| Children | 子女宫 | Offspring, creativity |
| Wealth | 财帛宫 | Money, finances |
| Health | 疾厄宫 | Illness, vulnerabilities |
| Travel | 迁移宫 | Movement, relocation |
| Friends | 交友宫 | Social circle |
| Career | 官禄宫 | Work, achievements |
| Property | 田宅宫 | Real estate, home |
| Fortune | 福德宫 | Happiness, spirit |
| Parents | 父母宫 | Parental relationships |

### The 14 Major Stars

**North Group (Authority)**:
- **Zi Wei** (紫微) — Emperor: Leadership, dignity
- **Tian Ji** (天机) — Strategist: Intelligence, planning
- **Tai Yang** (太阳) — Sun: Generosity, public life
- **Wu Qu** (武曲) — General: Finance, determination
- **Tian Tong** (天同) — Blessing: Comfort, ease
- **Lian Zhen** (廉贞) — Judge: Justice, complexity

**South Group (Nurturing)**:
- **Tian Fu** (天府) — Treasury: Wealth protection
- **Tai Yin** (太阴) — Moon: Intuition, sensitivity
- **Tan Lang** (贪狼) — Desire: Ambition, transformation
- **Ju Men** (巨门) — Gate: Communication, disputes
- **Tian Xiang** (天相) — Minister: Service, helpfulness
- **Tian Liang** (天梁) — Beam: Wisdom, problem-solving
- **Qi Sha** (七杀) — Warrior: Combat, adventure
- **Po Jun** (破军) — Pioneer: Change, rule-breaking

### The Four Transformations (四化)

| Hua | Chinese | Effect |
|-----|---------|--------|
| **Hua Lu** | 化禄 | Wealth/opportunity activation |
| **Hua Quan** | 化权 | Power/authority activation |
| **Hua Ke** | 化科 | Fame/recognition activation |
| **Hua Ji** | 化忌 | Obstruction/caution area |

See [references/zi-wei-dou-shu.md](references/zi-wei-dou-shu.md) for complete system details.

---

## BaZi (Four Pillars) Overview

BaZi uses four pillars from birth data:

| Pillar | Based On | Represents |
|--------|----------|------------|
| **Year** | Birth year | Outer self, grandparents |
| **Month** | Birth month | Parents, career |
| **Day** | Birth day | Self, spouse |
| **Hour** | Birth hour | Children, later life |

Each pillar has a **Heavenly Stem** (element + yin/yang) and **Earthly Branch** (animal).

```
Example: March 15, 1985, 3:00 AM

YEAR:   乙丑 (Yǐ Chǒu) - Yin Wood Ox
MONTH:  己卯 (Jǐ Mǎo) - Yin Earth Rabbit
DAY:    丁亥 (Dīng Hài) - Yin Fire Pig
HOUR:   壬寅 (Rén Yín) - Yang Water Tiger
```

See [references/bazi-guide.md](references/bazi-guide.md) for full BaZi system.

---

## Annual Forecasts

### Yearly Influences

Each year's animal affects all signs differently:

| Your Sign | Same Year | Clash Year | Harm Year | Punishment Year |
|-----------|-----------|------------|-----------|-----------------|
| Rat | Rat Year | Horse Year | Goat Year | Rabbit Year |
| Ox | Ox Year | Goat Year | Horse Year | Ox/Dog/Goat Year |
| Tiger | Tiger Year | Monkey Year | Snake Year | Tiger/Snake/Monkey Year |
| ... | ... | ... | ... | ... |

**Your Year (Ben Ming Nian)**: When your animal year comes around, wear red for protection. This year brings both challenges and opportunities.

---

## Navigation

**Resources**
- [references/zodiac-calculations.md](references/zodiac-calculations.md) — Finding animal signs
- [references/five-elements-guide.md](references/five-elements-guide.md) — Wu Xing deep dive
- [references/bazi-guide.md](references/bazi-guide.md) — Four Pillars system
- [references/zi-wei-dou-shu.md](references/zi-wei-dou-shu.md) — Purple Star Astrology (12 palaces, 14 stars)
- [references/compatibility-guide.md](references/compatibility-guide.md) — Relationship analysis
- [references/annual-forecast.md](references/annual-forecast.md) — Yearly predictions
- [references/chinese-calendar.md](references/chinese-calendar.md) — Lunar calendar system
- [references/bazi-day-master-analysis.md](references/bazi-day-master-analysis.md) — **Day Master strength, career/relationship guidance**
- [references/flying-stars-feng-shui.md](references/flying-stars-feng-shui.md) — **Flying Stars, annual charts, Feng Shui sectors**
- [references/qi-men-dun-jia.md](references/qi-men-dun-jia.md) — **Strategic timing, Eight Gates, direction selection**

**Templates**
- [assets/template-chinese-reading.md](assets/template-chinese-reading.md) — Full reading format
- [assets/template-compatibility.md](assets/template-compatibility.md) — Compatibility report

---

## Authoritative Sources

- Derek Walters, "The Chinese Astrology Bible"
- Lillian Too, "The Complete Illustrated Guide to Feng Shui"
- [Chinese Fortune Calendar](https://www.chinesefortunecalendar.com/)
- [YourChineseAstrology](https://www.yourchineseastrology.com/)
- Traditional Chinese almanac (Tung Shing / 通書)

---

## Trend Awareness Protocol

**IMPORTANT**: When users ask about missing features, what to add, or gaps in coverage, you MUST use WebSearch to check current trends before answering.

### Required Searches

1. Search: `"Chinese astrology BaZi apps 2026 features"`
2. Search: `"Chinese astrology TikTok viral 2026"`
3. Search: `"BaZi AI features trending"`
4. Search: `"Feng Shui app popular features"`

### What to Report

After searching, provide:

- **What's trending** and WHY it's popular (viral potential, user demand)
- **Top app features** that are driving downloads
- **Comparison** against current skill/app implementation
- **Priority recommendations** ranked by impact vs. effort

### Example Trending Topics (verify with fresh search)

- AI BaZi integration (ChatGPT + BaZi charts)
- Daily luck scores (career/wealth/health/relationship)
- Year-specific content (current Chinese zodiac year)
- Lucky directions calculator by birth chart
- Flying Stars annual updates
