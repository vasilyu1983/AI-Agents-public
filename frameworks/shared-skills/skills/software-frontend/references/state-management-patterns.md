...
| React Compiler | Automatic memoization — GA since Oct 2025 (v1.0), but opt-in per build tool (Babel/Vite/Rsbuild/Next.js), not on by default |
...
```typescript
// React Compiler reached GA (v1.0) in October 2025 and is opt-in per build
// tool (Babel, Vite, Rsbuild, or Next.js's `reactCompiler: true` flag) —
// it is not enabled automatically just by using a recent React or
// Next.js version. Once enabled, manual memoization is often unnecessary
// because the compiler automatically memoizes components, values, and callbacks
...
- [../../software-clean-code-standard/references/testing-utilities.md](../../software-clean-code-standard/references/testing-utilities.md) -- MSW for mocking API in state tests
