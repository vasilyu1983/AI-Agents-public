# Angular Best Practices & Patterns (Angular 19–22)

Operational patterns for production-ready Angular with standalone components, signals, and modern APIs. Written against Angular 19–21; Angular 22 (released June 2026) is now the current stable major, with Angular 21 in LTS. The patterns below carry forward unchanged into Angular 22 — standalone-by-default, signals, zoneless change detection, and the modern control-flow syntax are all still current. Angular 22 additionally stabilized Signal Forms, the `resource()`/`httpResource()` async APIs, and Angular Aria; treat any 22-specific API not covered here as needing a docs check against [angular.dev](https://angular.dev) before relying on it. Re-verify the current major at [angular.dev/reference/releases](https://angular.dev/reference/releases) — Angular ships a new major roughly every 6 months.
...
- [Signal-Based Forms (Simple Cases)](#signal-based-forms-simple-cases)
...
- [@Input/@Output to Signal APIs](#@input@output-to-signal-apis)
