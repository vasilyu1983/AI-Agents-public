...
### React Compiler (GA since Oct 2025 — v1.0; still opt-in, not automatic by default)

React Compiler reached stable 1.0 in October 2025 and is production-ready, but it is **not enabled automatically** — it must be turned on explicitly per build tool. It works with Babel, Vite, and Rsbuild in addition to Next.js; the Next.js config flag below is one integration path, not the only one.

```typescript
// next.config.ts (Next.js 16.1+)
const nextConfig: NextConfig = {
  reactCompiler: true, // opt-in — not the default even on current Next.js
};
