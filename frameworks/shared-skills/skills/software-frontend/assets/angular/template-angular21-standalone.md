# Angular Standalone Components Starter Template (Angular 21/22)

Production-ready template for building modern Angular applications with standalone components, zoneless change detection (stable and default for new apps), signals, and TypeScript. Written against Angular 21; carries forward unchanged to Angular 22 (current stable major as of June 2026) — re-check angular.dev if you need 22-specific APIs like Signal Forms or Angular Aria.
...
- **Angular 21** - Latest Angular with standalone components, signals, and modern CLI tooling
...
- **Vitest** - Unit testing for new Angular CLI projects
...
```bash
# Create new project (standalone components are the default)
...
# Add Tailwind CSS v4 (optional)
npm install -D tailwindcss @tailwindcss/postcss postcss
# Then configure PostCSS per the current Tailwind CSS v4 guide.
...
|-- tailwind.config.js               # Tailwind config (optional in v4)
`-- package.json
```
...
- **Standalone components are the default** since Angular 19+
- **Zoneless is stable and the default change-detection strategy for new apps** as of Angular 21+ (stabilized in v20.2); enable it explicitly with `provideZonelessChangeDetection()` and check third-party library compatibility first when adding it to an existing app
