...
1. **Run-to-run stochasticity**: the same prompt with the same inputs can succeed or fail on different runs due to sampling temperature, model nondeterminism, and context sensitivity. There is no "time-between-failures" — each run has a task-specific success probability, but common tools, prompts, environments and time windows can induce dependence; stochasticity does not establish independent Bernoulli trials.
...
| **pass^k** | All k runs succeed (conjunction) | Consistency metric; k is the chosen consistency horizon, not a universal sample-size threshold. Under iid independent repeated runs with constant success probability .90, all-ten success is .90^10≈.35; dependent repetitions require joint measurement and can differ |

**Decision threshold**: choose acceptable all-k success from the use case and loss consequences; no universal pass^10=.80 cutoff applies. (Gupta 2026 found perturbations dropped pass@1 from 96.9% to 88.1% — iid all-k success would decrease with k; dependence must be measured.)
...
For a sequential agent with N tool calls or steps, all-success reliability follows the conditional chain rule:

```
R_system = ∏ P(success_i | all prior steps successful)
Marginal r_i products require independence; a Markov assumption needs validation.
```
...
MAST (Cemri et al., arXiv:2503.13657 — 1,600+ traces across 7 frameworks, 14 failure modes, κ = 0.88 on the taxonomy) illustrates labeled failure counts in Figure 4 on 210 traces (first 30 per system), rather than all 1,642 dataset traces:

| Category | Share of illustrated labeled failures (Figure 4,210 traces) | Examples |
|---|---|---|
...
| Task verification | 21.3% | absent or inadequate verification of the final artefact |

[MAST PDF Figure 4, p.8](https://arxiv.org/pdf/2503.13657) verifies 41.8%/36.9%/21.3% for this illustrated subset. These are study-specific labeled-failure shares, not population failure probabilities or causal attribution. They do not prove that per-agent improvements leave 42% of failures untouched.

**Practical consequences:**

- Use conditional step-success probabilities or directly measured end-to-end success. Marginal products are valid only with justified independence and are neither a general upper nor lower bound.
- Enumerate handoff points as FMEA line items in their own right, not as properties of the adjacent agents.
- Compare centralized and peer validation using measured detection/repair coverage, common-error dependence, cost, and coordinator failure/recovery. A central validator can reduce some propagation while adding a single point of failure; topology alone proves no reliability ranking.
...
| Context loss or format mismatch at agent handoff | High — downstream agent treats a degraded artefact as ground truth | Multi-agent topologies with inadequate semantic/format checks | Validate schema and task-relevant semantic evidence at the handoff boundary; compare centralized and peer validation using measured coverage, common-error dependence and coordinator failure/recovery rather than assuming a topology preference |
...
- [ ] **Agent reliability target must be set**: state per-run success and confidence, with pass^k at a chosen consistency horizon; distinguish k from total sampled episodes.
...
- [ ] **More than one agent in the system**: model conditional handoff success; do not treat marginal ∏ rᵢ as a universal bound; enumerate handoff points as their own FMEA items; check for a missing termination condition.
...
5. Yan, H. et al. (2025). "An Empirical Study of Production Incidents in Generative AI Cloud Services." ISSRE 2025. arXiv:2504.08865. _(already in sources.json — normalized average TTM differs; displayed 1.12/.65 and text 1.83 have unresolved discrepancy)_
