...
- [Contents](#contents)
- [Operational Knowledge Loop Pattern](#operational-knowledge-loop-pattern)
...
- [Zero-Trust Security Pattern](#zero-trust-security-pattern)
...
> Note: many examples below intentionally show lower-level request-handler patterns. For new servers, start with the higher-level `McpServer` / `FastMCP` guidance in `references/mcp-custom.md`, then drop down to these patterns only when you need finer protocol control.

## Contents

- Operational Knowledge Loop Pattern
...
Use this pattern when the agent needs to combine a canonical note system with fresh operational artifacts.

Typical shape:

- one connector for the canonical store, such as a local note vault
- one connector for ingest artifacts, such as Drive or a watched export folder
- optional connectors for current work surfaces such as Slack or Calendar
- one explicit write-back policy that says which system is allowed to become the durable record

Design rules:

- keep one canonical memory surface
- treat transcripts, exports, and messages as evidence, not as the final memory layer
- default connectors to read-only until the routing rules are stable
- if write-back is allowed, restrict it to a narrow set of destinations
- never let every connector write everywhere

This pattern is a good fit for "AI chief of staff" style workflows without inventing a new protocol surface.
...
For enterprise multi-agent deployments, multi-agent systems coordinate across multiple MCP servers.
