...
> **Versioning note (SLSA v0.1 → v1.0+):** SLSA v0.1 defined four levels (1–4). In SLSA v1.0 (2023) the numbering was reset to L1–L3; the former "SLSA 4" (hermetic + reproducible builds) was folded into Build L3 as additional hardening requirements rather than a separate level. Prefer the v1.x terminology. See [SLSA v1.2 spec](https://slsa.dev/spec/v1.2/) (cited in `data/sources.json`).

| Level (SLSA v1.x) | Former v0.1 Equivalent | Requirements | Trust |
|-------------------|------------------------|--------------|-------|
| **Build L1** | SLSA 1 | Documented build process, scripted build | Low |
| **Build L2** | SLSA 2 | Version-controlled source, hosted build service | Medium |
| **Build L3** | SLSA 3 + SLSA 4 | Hardened build platform, signed provenance, hermetic + reproducible builds | High |
...
- [SLSA v1.2](https://slsa.dev/spec/v1.2/) — Supply chain security specification
