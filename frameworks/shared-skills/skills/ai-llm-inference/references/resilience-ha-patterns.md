...
- [US-WEST region](#us-west-region)
- [US-EAST region (identical config, different nodeSelector)](#us-east-region-identical-config-different-nodeselector)
- [Active-Passive Architecture](#active-passive-architecture)
- [Health check and failover script](#health-check-and-failover-script)
- [Monitor loop](#monitor-loop)
- [Health Probes & Circuit Breakers](#health-probes-&-circuit-breakers)
- [Model warm status](#model-warm-status)
- [Usage](#usage)
- [Autoscaling Tied to SLOs](#autoscaling-tied-to-slos)
- [SLO-Based Autoscaling](#slo-based-autoscaling)
- [Pre-warming Replicas](#pre-warming-replicas)
- [Block Deploys When SLOs Regress](#block-deploys-when-slos-regress)
- [GitLab CI example](#gitlab-ci-example)
- [scripts/validate_slo.py](#scriptsvalidateslopy)
- [Overload Protection & Backpressure](#overload-protection-&-backpressure)
- [Token-Level Rate Limiting](#token-level-rate-limiting)
- [Token budget per user (tokens/minute)](#token-budget-per-user-tokensminute)
- [Queue Caps & Request Shedding](#queue-caps-&-request-shedding)
- [Graceful Degradation](#graceful-degradation)
- [Priority-Based Load Shedding](#priority-based-load-shedding)
- [Traffic Shaping & Safe Rollouts](#traffic-shaping-&-safe-rollouts)
- [Canary Deployments](#canary-deployments)
- [Stable deployment (90% traffic)](#stable-deployment-90%-traffic)
- [Canary deployment (10% traffic)](#canary-deployment-10%-traffic)
- [Service routes to both](#service-routes-to-both)
- [Start: 10% canary](#start-10%-canary)
- [Monitor metrics for 30 minutes](#monitor-metrics-for-30-minutes)
- [If healthy: 50% canary](#if-healthy-50%-canary)
- [Monitor for 30 minutes](#monitor-for-30-minutes)
- [If healthy: 100% canary](#if-healthy-100%-canary)
- [If unhealthy: Rollback](#if-unhealthy-rollback)
- [Shadow Traffic](#shadow-traffic)
- [Weighted Routing](#weighted-routing)
- [Sticky Sessions (KV Cache Reuse)](#sticky-sessions-kv-cache-reuse)
- [Kubernetes service with session affinity](#kubernetes-service-with-session-affinity)
- [Map session ID to replica](#map-session-id-to-replica)
- [Stateful Asset Coordination](#stateful-asset-coordination)
- [KV Cache Invalidation](#kv-cache-invalidation)
- [Call during deployment](#call-during-deployment)
- [Prompt & Index Version Pinning](#prompt-&-index-version-pinning)
- [Atomic Rollout/Rollback](#atomic-rolloutrollback)
- [Deploy green (new version)](#deploy-green-new-version)
- [Wait for health checks](#wait-for-health-checks)
- [Switch traffic (instant cutover)](#switch-traffic-instant-cutover)
- [Rollback if needed](#rollback-if-needed)
- [Checklist: High Availability Ready](#checklist-high-availability-ready)
