...
- [Expert Parallelism (EP) - Mixture of Experts](#expert-parallelism-ep-mixture-of-experts)
...
- [TP + PP (Most Common)](#tp-pp-most-common)
- [TP + EP (MoE Models)](#tp-ep-moe-models)
...
### Reference Diagram — How each split shards the work

Same model, four ways to partition. Boxes show what each GPU holds; arrows show the collective that pays for it.

```mermaid
flowchart TB
    subgraph TP["Tensor Parallelism (TP) — split a layer"]
        direction LR
        L_TP[Layer L] --> G1_TP[GPU 1<br/>columns 0..N/2]
        L_TP --> G2_TP[GPU 2<br/>columns N/2..N]
        G1_TP --> AR_TP[All-Reduce per layer<br/>needs NVLink-class bw]
        G2_TP --> AR_TP
    end

    subgraph PP["Pipeline Parallelism (PP) — split layers"]
        direction LR
        I_PP[Input batch] --> S1[Stage 1<br/>layers 1..L/4] --> S2[Stage 2<br/>layers L/4..L/2] --> S3[Stage 3<br/>layers L/2..3L/4] --> S4[Stage 4<br/>layers 3L/4..L]
        S4 -. micro-batch pipeline<br/>bubble overhead .-> I_PP
    end

    subgraph EP["Expert Parallelism (EP) — split MoE experts"]
        direction LR
        T_EP[Token batch] --> R_EP[Router]
        R_EP --> E1[GPU 1<br/>experts 1..N/4]
        R_EP --> E2[GPU 2<br/>experts N/4..N/2]
        R_EP --> E3[GPU 3<br/>experts N/2..3N/4]
        R_EP --> E4[GPU 4<br/>experts 3N/4..N]
        E1 --> AA_EP[All-to-All<br/>token routing]
        E2 --> AA_EP
        E3 --> AA_EP
        E4 --> AA_EP
    end

    subgraph DP["Data Parallelism (DP) — replicate, split batch"]
        direction LR
        B_DP[Global batch] --> B1[Shard 1] --> R1[Full model replica 1]
        B_DP --> B2[Shard 2] --> R2[Full model replica 2]
        B_DP --> B3[Shard 3] --> R3[Full model replica 3]
        R1 -. all-reduce grads<br/>training only .-> R2
        R2 -. .-> R3
    end
```

Inference picks: **TP** for latency on a single replica; **PP** when the model doesn't fit even with TP; **EP** for MoE models; **DP** for throughput across replicas. Hybrid stacks combine them (`TP + PP + EP` is standard for frontier-tier MoE serving).
...
**Best for**:
- MoE models (Mixtral, DeepSeek-V3/R1, Qwen3-MoE, and other sparse-expert architectures — verify the specific model's routed/active-expert design before sizing EP degree)
...
Starting-point heuristics, not measured benchmarks — GPU count depends on precision, context length, and concurrency target as much as parameter count. Re-derive from the KV-cache and weight-memory math in [kv-cache-optimization.md](kv-cache-optimization.md) before committing to a topology.
...
# Illustrative shape of results only — these are NOT measured or benchmark-sourced numbers.
# Actual tok/s depends on model size, precision, batch size, prompt/output length, and
# interconnect. Run this script against your own model and hardware to get real numbers.
# What to look for: TP=N throughput should land close to N x single-GPU throughput
# (efficiency = actual / (N x single-GPU baseline)); large gaps below ~80% efficiency on
# NVLink-class interconnects usually indicate a communication or load-balance problem,
# not an inherent TP ceiling.
```
