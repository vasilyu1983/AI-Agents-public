...
- [Leveraging TM for Cost Savings](#leveraging-tm-for-cost-savings)
- [TM Quality Management](#tm-quality-management)
- [TM File Formats](#tm-file-formats)
- [Glossary Management](#glossary-management)
- [Why Glossaries Matter](#why-glossaries-matter)
- [Glossary Structure](#glossary-structure)
- [Example Glossary Entries](#example-glossary-entries)
- [Domain-Specific Terminology](#domain-specific-terminology)
- [Glossary Maintenance Workflow](#glossary-maintenance-workflow)
- [Context for Translators](#context-for-translators)
- [Why Context Reduces Rework](#why-context-reduces-rework)
- [Context Types](#context-types)
- [Providing Context in Code](#providing-context-in-code)
- [Screenshot Automation](#screenshot-automation)
- [String Key Naming Conventions](#string-key-naming-conventions)
- [Convention Comparison](#convention-comparison)
- [Recommended Convention](#recommended-convention)
- [Key Naming Rules](#key-naming-rules)
- [Key Design Anti-Patterns](#key-design-anti-patterns)
- [Handling Dynamic Content](#handling-dynamic-content)
- [Content Types and Strategies](#content-types-and-strategies)
- [CMS Integration Patterns](#cms-integration-patterns)
- [Legal and Compliance Content](#legal-and-compliance-content)
- [Version Control for Translations](#version-control-for-translations)
- [Branching Strategies](#branching-strategies)
- [Release-Based Workflow](#release-based-workflow)
- [Machine Translation Quality Gates](#machine-translation-quality-gates)
- [Git Workflow for Translation Files](#git-workflow-for-translation-files)
- [.gitattributes — mark translation files for merge strategy](#gitattributes-—-mark-translation-files-for-merge-strategy)
- [Alternatively, exclude translation files from normal diff](#alternatively-exclude-translation-files-from-normal-diff)
- [Quality Assurance](#quality-assurance)
- [Linguistic QA Tools](#linguistic-qa-tools)
- [QA Check Categories](#qa-check-categories)
- [In-Context Review](#in-context-review)
- [Back-Translation](#back-translation)
- [Cost Optimisation](#cost-optimisation)
- [Reuse Rate Benchmarks](#reuse-rate-benchmarks)
- [Cost Reduction Strategies](#cost-reduction-strategies)
- [Batch vs Continuous Translation](#batch-vs-continuous-translation)
- [Machine Translation + Human Review (MTPE)](#machine-translation-human-review-mtpe)
- [MTPE Quality Tiers](#mtpe-quality-tiers)
- [MT Engine Comparison](#mt-engine-comparison)
- [MTPE Workflow](#mtpe-workflow)
- [Cost Model Example](#cost-model-example)
...
// Extractors such as i18next-cli or legacy i18next-parser can pick these comments up
t('save_button', {
...
Avoid overloading one key for multiple UI jobs.

| Pattern | Status | Why |
|---------|--------|-----|
| `meanings.4.advice` reused for badges, modal descriptions, affirmations, and card copy | FAIL | Any one copy change breaks the others |
| `meanings.4.advice`, `lifePath.4.affirmation`, `birthday.4.guidance` | PASS | Each surface can evolve independently |
| Single `nodeGrowth` key used in both a long paragraph and a badge | FAIL | Substring hacks break in non-English locales |
| `nodeGrowth` + `nodeGrowthShort` | PASS | Explicit short and long variants |

When API output format does not match locale key naming, use an explicit map instead of string transforms:
...
**EU market note**: The European Accessibility Act (EAA) has been enforceable since 28 June 2025 for products/services placed on the EU market after that date (a transition period to 28 June 2030 applies to services already on the market before then). The EAA itself is an accessibility law, not a language law — it does not mandate translation. But it interlocks with rules that do: the Web Accessibility Directive (public sector), the EU Consumer Rights Directive (consumer information in the member-state language), and national transpositions that often add their own linguistic obligations. Practical effect for this skill: e-commerce, banking, transport, and media products selling into the EU need both accessible markup (see `software-accessibility`) *and* accurate localisation in each served market's language — treat the two as one compliance review, not two separate checklists, and confirm current scope/deadlines with counsel rather than this note, since enforcement details continue to evolve per member state.
...
2. Strings extracted at release branch cut
   └─ npx i18next-cli extract --ci → diff shows new/changed keys
...
Short, domain-specific terms are easy to mistranslate. Keep a locale glossary before using MT at scale.

| Locale | Expected | Common Bad MT | Term |
|--------|----------|---------------|------|
...
1. Maintain a curated dictionary of domain terms per locale.
2. Never auto-translate terms shorter than 3 words without glossary lookup.
3. Grep for known-bad translations after MT and block release on matches.
4. Translate glossary terms first, then use them as constraints for MT or AI-assisted translation.
...
Per-word rates below are illustrative placeholders to show the savings mechanics, not current market pricing — actual per-word rates vary by language pair, vendor, and content complexity and should be re-quoted from your own vendor before budgeting.
