...
**Adaptive Delay mode** (recent Clang TSan): `TSAN_OPTIONS=enable_adaptive_delay=1` injects delays at synchronization points to perturb thread scheduling and expose additional races (tune with `adaptive_delay_aggressiveness` and per-operation sample-rate flags). Disabled by default; useful when a race is suspected but the standard run does not trigger it. (Source: clang.llvm.org/docs/ThreadSanitizer.html, verified 2026-07-11.)
...
# Use thread-aware logging instead of private runtime flags
import logging
logging.basicConfig(
    level=logging.DEBUG,
