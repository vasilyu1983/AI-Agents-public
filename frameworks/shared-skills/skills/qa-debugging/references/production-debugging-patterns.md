...
rg "<YYYY-MM-DDTHH:" /var/log/app/app.log --no-filename --no-line-number | \
  rg "ERROR" --no-filename --no-line-number
...
jq 'select(.level == "error" and .duration > 1000)' app.log

# Find payment failures
jq 'select(.message | contains("payment")) | select(.level == "error")' app.log

# Group errors by error.code
jq -r 'select(.level == "error") | .error.code' app.log | \
  sort | uniq -c | sort -rn
```
...
- Datadog APM
- New Relic
- Jaeger (v2) / Zipkin — note: Jaeger v1 EOL December 2025
- AWS X-Ray
- Google Cloud Trace
