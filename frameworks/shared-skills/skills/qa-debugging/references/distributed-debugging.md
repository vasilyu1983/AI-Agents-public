...
# Prefer sending OTLP to a Collector, then fan out to Jaeger/Tempo/etc.
...
order-service
  ...
  ├── inventory-service (p99: 120ms, error: 0.5%)  (!) High latency
  ├── payment-service (p99: 800ms, error: 2.1%)    (X) High errors
...
|------|------|----------|------------|
| Jaeger | Open source | Full-featured distributed tracing (v2 built on OTel Collector; v1 EOL Dec 2025) | Self-hosted, K8s operator |
...
Jaeger v1 reached end-of-life December 2025. Use Jaeger v2, which is built on the OpenTelemetry Collector framework. The `jaegertracing/all-in-one` image is now the legacy v1 image; use `jaegertracing/jaeger` for v2.

```yaml
# docker-compose.yml for local development (Jaeger v2)
services:
  jaeger:
