...
# For intermittent production slowness, prefer continuous profiling
# (for example Pyroscope/Parca) over one-off captures
```
...
# Filter by an explicit hour window (portable across shells/OSes)
rg "<YYYY-MM-DDTHH:" /var/log/app/app.log --no-filename --no-line-number | rg "ERROR" --no-filename --no-line-number
...
describe('Timezone handling', () => {
  ...
  test('handles DST spring forward', () => {
    // DST spring-forward hour doesn't exist in America/New_York
...
- **Pyroscope / Parca** - Continuous profiling for intermittent issues
