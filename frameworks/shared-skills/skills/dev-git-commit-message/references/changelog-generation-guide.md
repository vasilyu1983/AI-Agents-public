...
4. [Setup: release-please](#setup-release-please)
5. [Setup: semantic-release](#setup-semantic-release)
6. [Setup: changesets](#setup-changesets)
7. [Output Format: Keep a Changelog](#output-format-keep-a-changelog)
8. [Customization](#customization)
9. [CI Integration](#ci-integration)
10. [Manual Overrides](#manual-overrides)
11. [Versioning Integration](#versioning-integration)
12. [Example Workflow](#example-workflow)
13. [Do / Avoid](#do--avoid)
14. [Checklist: Changelog Release](#checklist-changelog-release)
...
| standard-version *(deprecated 2022)* | Yes | Yes | No | Limited | Yes (review before push) |
| release-please | Yes | Yes | Optional | Via manifest | Yes (release PR review) |
...
- Need GitHub-native release PRs with a review gate? Use **release-please**.
- Want to review changelog before release? Use **release-please** or **changesets**.
...
**⚠ DEPRECATED since 2022 — do not adopt for new projects.** `standard-version` carries an official deprecation notice on its README pointing adopters to `release-please` (GitHub-native repos) or the community-maintained fork `commit-and-tag-version` (non-GitHub or self-hosted CI). It is not fully archived — the fork keeps taking patches — but treat any new `standard-version` dependency as tech debt on day one.

---

## Setup: release-please

> **Version currency (verify before pinning):** `googleapis/release-please-action` reached v5 in April 2026 (breaking change: requires the Node 24 GitHub Actions runtime); the underlying `release-please` npm package is on the v17.x line. The v3→v4 change already retired the standalone `release-type` input and `command: manifest` in favor of a `release-please-config.json`-driven manifest strategy — that config-file approach is still current in v5. Because this tool ships major versions faster than most release tooling, check the [releases page](https://github.com/googleapis/release-please-action/releases) before pinning a workflow tag; do not trust a hardcoded major version in this guide.

Best fit for GitHub repositories that want automated versioning and changelog updates behind a release PR.
...
jobs:
  release-please:
    ...
    steps:
      - uses: googleapis/release-please-action@v5  # verify latest tag before pinning
        with:
          release-type: simple
```

Manifest-based repos can use `.release-please-manifest.json` plus `release-please-config.json` to manage multiple packages or components.

Why choose it:

- GitHub-native release PR workflow
- Human review before tagging and publishing
- Good default for new GitHub repos that want automation without fully hands-off releases
...
- **auth**: session-based tokens removed; migrate to JWT (see your project migration guide)
...
Control which types appear and under what heading using `.versionrc` (standard-version) or `presetConfig` (semantic-release or release-please-compatible generators):
...
After running `standard-version --dry-run`, reviewing a release-please PR, or running `changeset version`:
...
See your project's migration guide or release notes appendix.
```
