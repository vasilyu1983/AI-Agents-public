...
**Custom types**: Local extensions such as `security` can be valid inside a specific repository, but they are not part of the base Conventional Commits specification. Prefer standard types plus a scope unless the repository already enforces the custom type.
