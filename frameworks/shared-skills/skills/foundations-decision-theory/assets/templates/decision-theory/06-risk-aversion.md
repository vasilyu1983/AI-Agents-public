...
CE and Arrow-Pratt curvature alone do not specify a repeated wealth process. Model wealth transitions, correlation, leverage and absorbing boundaries explicitly. Expected utility can evaluate terminal wealth or path-dependent outcomes with a suitable utility function; expected log growth is one coherent growth objective, not a universal replacement. Positive arithmetic return can coexist with negative long-run log growth even without literal one-period ruin. Survival/drawdown constraints must reflect the decision maker's stated requirements.
