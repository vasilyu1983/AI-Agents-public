...
- [Choose the right surface first](#choose-the-right-surface-first)
- [Contents](#contents)
- [Core endpoints (most common)](#core-endpoints-most-common)
- [Card payload shape (practical subset)](#card-payload-shape-practical-subset)
- [Query types](#query-types)
- [Native SQL card example](#native-sql-card-example)
- [Editing workflow (recommended)](#editing-workflow-recommended)
- [Query Builder (MBQL) card example](#query-builder-mbql-card-example)
- [Parameters and safe query templating](#parameters-and-safe-query-templating)
- [ID discovery cheatsheet](#id-discovery-cheatsheet)
- [Executing queries and exporting results](#executing-queries-and-exporting-results)
...
|------|-------------------|
| Create or edit saved questions | Classic Metabase REST API (`/api/card`) |
| Manage `visualization_settings` and collections | Classic Metabase REST API |
| Run semantic, AI-driven query workflows | Agent API |
...
Note: Exact endpoints and response shapes can vary by Metabase version. Confirm on your instance by exporting an existing card via `GET /api/card/:id` and editing that JSON.
...
If the goal is not "save a question in Metabase" but "let an application-side AI agent discover metrics and construct queries", prefer the Agent API instead of manufacturing cards.
...
Tip: the helper script now exposes `list-collections`, `list-databases`, `database-metadata`, and `list-fields` so you can discover IDs without manual UI clicks.
