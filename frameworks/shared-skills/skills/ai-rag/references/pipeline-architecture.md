...
- [End-to-End RAG Pipeline Structure](#end-to-end-rag-pipeline-structure)
- [Reference Diagram](#reference-diagram)
- [Pipeline Stages](#pipeline-stages)
- [Pipeline Implementation Checklist](#pipeline-implementation-checklist)
- [Ingestion, Freshness, and Index Hygiene](#ingestion-freshness-and-index-hygiene)
- [Data Feeds & Ingestion Patterns](#data-feeds--ingestion-patterns)
- [Ingestion Health Checklist](#ingestion-health-checklist)
- [Embedding Model Selection](#embedding-model-selection)
- [Decision Rules](#decision-rules)
- [Embeddings Ready Checklist](#embeddings-ready-checklist)
...
Two flows on one canvas: build-time ingestion (top) and query-time retrieval-and-generation (bottom). They share the vector index but otherwise run independently.

```mermaid
flowchart TB
    subgraph Ingest["Build-time ingestion"]
        direction LR
        SRC[Sources<br/>PDFs / wikis / DBs] --> EX[Extract + clean]
        EX --> CH[Chunk<br/>fixed / semantic / contextual]
        CH --> EMB[Embedding model]
        EMB --> META[Attach metadata<br/>source, ts, ACL, lang]
        META --> IDX[(Vector index<br/>HNSW / IVF)]
        META --> KW[(BM25 / lexical index)]
    end

    subgraph Query["Query-time retrieval and generation"]
        direction LR
        Q[User query] --> QR[Query rewrite<br/>HyDE / multi-query / decomposition]
        QR --> QE[Embed query]
        QE --> VS[Vector search top-k]
        QR --> BS[BM25 search top-k]
        IDX --> VS
        KW --> BS
        VS --> FUSE[Fusion<br/>RRF / weighted]
        BS --> FUSE
        FUSE --> RR[Reranker<br/>cross-encoder]
        RR --> FILT[Filter + dedupe<br/>by ACL, freshness, source]
        FILT --> PACK[Context packager<br/>token budget, ordering]
        PACK --> GEN[LLM generation<br/>with grounding rules]
        GEN --> CITE[Cite + verify<br/>span-level attribution]
        CITE --> ANS[Response]
        ANS -. user feedback .-> FB[(Feedback store)]
        FB -. retrain reranker / rewrite .-> RR
    end
```
