...
**Freshness anchor:** 2026-07-11 — `Retry-After` remains the stable minimum; the IETF `RateLimit`/`RateLimit-Policy` fields are at draft-11 (2026-05-23) and still an Internet-Draft, not an RFC, while legacy `X-RateLimit-*` headers remain widely deployed.
...
- [Algorithm 1: Token Bucket (Recommended)](#algorithm-1-token-bucket-recommended)
- [Algorithm 2: Fixed Window](#algorithm-2-fixed-window)
- [Algorithm 3: Sliding Window](#algorithm-3-sliding-window)
- [Algorithm 4: Leaky Bucket](#algorithm-4-leaky-bucket)
- [Rate Limit Headers](#rate-limit-headers)
- [Tiered Rate Limits](#tiered-rate-limits)
- [Implementation Examples](#implementation-examples)
- [Redis + Token Bucket](#redis-token-bucket)
- [Redis + Sliding Window Log](#redis-sliding-window-log)
- [Express.js + express-rate-limit](#expressjs-express-rate-limit)
- [Per-Endpoint Rate Limits](#per-endpoint-rate-limits)
- [Rate Limiting Strategies](#rate-limiting-strategies)
- [Per User/API Key](#per-userapi-key)
- [or](#or)
- [Per IP Address](#per-ip-address)
- [Per User + Endpoint](#per-user-endpoint)
- [Best Practices Checklist](#best-practices-checklist)
- [Common Anti-Patterns](#common-anti-patterns)
- [BAD: No Rate Limiting](#bad-no-rate-limiting)
- [Bad - No protection against abuse](#bad-no-protection-against-abuse)
- [BAD: Per-IP Only (Not Per-User)](#bad-per-ip-only-not-per-user)
- [Bad - Users behind NAT blocked together](#bad-users-behind-nat-blocked-together)
- [BAD: Missing Retry-After Header](#bad-missing-retry-after-header)
- [BAD: No Burst Allowance](#bad-no-burst-allowance)
- [Advanced Patterns](#advanced-patterns)
- [Adaptive Rate Limiting](#adaptive-rate-limiting)
- [Rate Limit Bypass for Trusted Clients](#rate-limit-bypass-for-trusted-clients)
- [Cost-Based Rate Limiting](#cost-based-rate-limiting)
- [Consume tokens based on cost](#consume-tokens-based-on-cost)
...
**Document one header scheme clearly.**

As of July 2026:

- `Retry-After` is the stable baseline for 429 responses
- `RateLimit` / `RateLimit-Policy` are the direction of the current IETF draft
- `X-RateLimit-*` remains common in gateways and public APIs

**Example using the newer field names:**

```http
RateLimit: limit=1000, remaining=987, reset=60
RateLimit-Policy: 1000;w=60
Retry-After: 60
```

**Legacy example still widely seen:**
...
- [ ] **Rate limit headers included** (documented and consistent for your clients)
