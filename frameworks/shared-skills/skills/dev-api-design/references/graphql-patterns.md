...
- [Payload types for mutations](#payload-types-for-mutations)
- [Core Patterns](#core-patterns)
- [1. Connection Pattern (Pagination)](#1-connection-pattern-pagination)
- [2. Input Object Pattern](#2-input-object-pattern)
- [BAD: Bad - Too many arguments](#bad-bad-too-many-arguments)
- [GOOD: Good - Input object](#good-good-input-object)
- [3. Payload Pattern](#3-payload-pattern)
- [4. Interface Pattern](#4-interface-pattern)
- [5. Union Types Pattern](#5-union-types-pattern)
- [6. Enum Pattern](#6-enum-pattern)
- [7. Custom Scalar Pattern](#7-custom-scalar-pattern)
- [8. Directive Pattern](#8-directive-pattern)
- [9. DataLoader Pattern (N+1 Problem)](#9-dataloader-pattern-n1-problem)
- [Schema Design Best Practices](#schema-design-best-practices)
- [1. Use Nullable vs Non-Nullable Strategically](#1-use-nullable-vs-non-nullable-strategically)
- [2. Avoid Over-Nesting](#2-avoid-over-nesting)
- [BAD: Bad - Too deeply nested](#bad-bad-too-deeply-nested)
- [Leads to queries like:](#leads-to-queries-like)
- [3. Implement Query Complexity Analysis](#3-implement-query-complexity-analysis)
- [4. Use Consistent Naming](#4-use-consistent-naming)
- [5. Deprecate, Don't Remove](#5-deprecate-dont-remove)
- [GraphQL vs REST Comparison](#graphql-vs-rest-comparison)
- [Real-World GraphQL Subscriptions](#real-world-graphql-subscriptions)
- [Client subscribes](#client-subscribes)
- [Security Best Practices](#security-best-practices)
- [1. Authentication](#1-authentication)
- [2. Authorization (Field-Level)](#2-authorization-field-level)
- [3. Rate Limiting](#3-rate-limiting)
- [4. Query Timeouts](#4-query-timeouts)
- [Testing GraphQL APIs](#testing-graphql-apis)
- [Unit Tests (Resolvers)](#unit-tests-resolvers)
...
Federation v2 lets independent teams own subgraph schemas that compose into one unified supergraph. Key directives: `@key` marks entity primary keys for cross-subgraph resolution; `@shareable` allows a type to be resolved by multiple subgraphs; `@inaccessible` hides fields from the public schema while keeping them for internal use.

**Managed federation** (Apollo GraphOS) provides a schema registry with composition validation and safe schema rollouts. **Apollo Router** (Rust, open-source core) or **Cosmo Router** (WunderGraph, Apache 2.0, fully OSS including the control plane) handle supergraph execution at the edge.

As of mid-2026, Cosmo Router composes GraphQL, REST, gRPC, and Kafka into a single federated supergraph. It also ships native MCP integration, connecting the supergraph to agent tools (Cursor, Windsurf, VSCode). Benchmark: Cosmo reports better P99 latency and higher throughput than Apollo Router — verify against your workload.

**When to adopt Federation v2**: you have 3+ teams maintaining separate GraphQL services, or schema-stitching is causing resolver ownership conflicts and breaking changes. Avoid if you have a single team and one schema — federation adds operational overhead that pays off only at multi-team scale.

**Stay schema-stitched** for small organisations or monorepos where a shared schema with code-gen is simpler and breaking-change detection is handled by GraphQL Inspector or oasdiff-equivalent tooling.

**Verified:** 2026-06-09 against WunderGraph/Cosmo docs and GitHub activity.
