...
- [Contents](#contents)
- [Priority Levels Overview](#priority-levels-overview)
- [Priority 1: External-Facing (Must Document)](#priority-1-external-facing-must-document)
- [Priority 2: Internal Integration (Should Document)](#priority-2-internal-integration-should-document)
- [Priority 3: Developer Reference (Nice to Have)](#priority-3-developer-reference-nice-to-have)
- [Prioritization Decision Tree](#prioritization-decision-tree)
- [Documentation Coverage by Category](#documentation-coverage-by-category)
- [Documentation Debt Scoring](#documentation-debt-scoring)
- [Effort Estimation](#effort-estimation)
- [Compliance Considerations](#compliance-considerations)
- [Re-Prioritization Triggers](#re-prioritization-triggers)
- [Prioritizing by Usage Signal, Not Just Component Type](#prioritizing-by-usage-signal-not-just-component-type)
...
- Prioritizing by Usage Signal, Not Just Component Type
- Related Resources
...
The P1/P2/P3 tiers above are a starting floor based on *what kind* of component it is. Within
a tier — and sometimes across tiers — real usage evidence should move an item up the list
faster than its component type alone would suggest:

- **Support-ticket / Slack-question frequency**: pull a rough count of how often a topic
  generates a question in support channels, Slack, or the issue tracker over the last quarter.
  A P3 config option with a steady trickle of "what does this do" questions can outrank a P2
  gap nobody has asked about.
- **Onboarding friction**: if new hires independently ask the same question in their first two
  weeks (check onboarding retros or 30/60/90 check-ins), that is a documentation gap even
  without a formal ticket — new hires under-report friction as "tickets" because they assume
  it is their own inexperience, not a doc gap.
- **Incident postmortems**: any postmortem action item or contributing-factor note that cites
  "runbook was wrong," "docs didn't cover this," or "nobody knew where the setting lived"
  should immediately re-prioritize that doc to P1, regardless of its original tier, and should
  feed the escalation triggers above.
- **Absence of signal is not evidence of low priority**: a brand-new integration or a
  component ahead of a launch will have zero historical tickets by definition. Cross-check
  against the product roadmap before treating "no questions yet" as "low priority."

When usage signal and component-type priority disagree, report both in the coverage output
(for example: "P2 by type, P1 by ticket volume — 6 tickets/quarter") rather than silently
picking one. That preserves the reasoning for whoever triages the backlog next.
