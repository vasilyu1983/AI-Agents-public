...
### Test and QA Runbooks

Test runbooks need additional validation because stale commands waste engineering time and hide the real gate topology.

Required sections for a production-ready QA runbook:

- environment bootstrap or canonical dev-server command
- exact targeted spec or named-batch command
- cleanup/reset command for shared stateful dependencies
- deploy-gate replay command for final release confidence
- artifact locations for trace, logs, screenshots, and failure context

Test-runbook failures to treat as P1:

- stale “run the whole suite” guidance when the canonical workflow is targeted-first
- missing cleanup/reset step for stateful test environments
- missing artifact path for deflake or incident triage
- duplicate or contradictory runner instructions across docs
...
- **Draft** = Runbook exists but not yet tested in CI
