...
- [ViewMatchers (Find Elements)](#viewmatchers-find-elements)
- [ViewActions (Interact)](#viewactions-interact)
- [ViewAssertions (Verify)](#viewassertions-verify)
- [RecyclerView Testing](#recyclerview-testing)
- [Scroll and Click](#scroll-and-click)
- [Click Child View in Item](#click-child-view-in-item)
- [Assert Item Count](#assert-item-count)
- [Intents Testing](#intents-testing)
- [Stub External Intents](#stub-external-intents)
- [Verify Intent Was Sent](#verify-intent-was-sent)
- [Custom Matchers](#custom-matchers)
- [Text Color Matcher](#text-color-matcher)
- [Drawable Matcher](#drawable-matcher)
- [EditText Error Matcher](#edittext-error-matcher)
- [Handling Async Operations](#handling-async-operations)
- [IdlingResource Pattern](#idlingresource-pattern)
- [CountingIdlingResource](#countingidlingresource)
- [Robot Pattern (Full Example)](#robot-pattern-full-example)
- [Test Annotations and Rules](#test-annotations-and-rules)
- [Disable Animations Rule](#disable-animations-rule)
- [Grant Permissions Rule](#grant-permissions-rule)
- [Debugging Tips](#debugging-tips)
- [Print View Hierarchy](#print-view-hierarchy)
...
> **Trap: forgetting to register a custom IdlingResource**
>
> Creating an `IdlingResource` implementation without registering it with
> `IdlingRegistry.getInstance().register(...)` is the #1 cause of Espresso flakiness.
> Espresso only waits for resources it knows about; an unregistered resource is invisible to
> the synchronisation loop.
>
> **Symptom**: tests pass reliably on a developer machine (fast device, warm cache) but flake
> in CI under load — typically manifesting as `NoMatchingViewException` or an assertion on
> a stale loading state. The failure is not reproducible by re-running locally.
>
> **Root cause**: Espresso considers the UI idle as soon as the main thread is idle, but the
> app has outstanding async work tracked by the custom resource that Espresso does not know about.
> On fast hardware the async work finishes before Espresso acts; on slow CI it does not.
>
> **Fix**: register every custom async source in `@Before` and unregister in `@After`.
>
> ```kotlin
> @Before
> fun setUp() {
>     // Must be called — creating the resource is not enough
>     IdlingRegistry.getInstance().register(EspressoIdlingResource.countingIdlingResource)
> }
>
> @After
...
> ```
>
> **Migration path for Compose**: if the async work is encapsulated in a Compose-native
> ViewModel or repository, prefer `composeTestRule.waitUntil { ... }` or
> `composeTestRule.registerIdlingResource(resource)` over the Espresso registry.
> This removes the Espresso dependency while preserving deterministic waiting.
