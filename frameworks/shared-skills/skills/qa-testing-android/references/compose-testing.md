...
- [By TestTag (Recommended)](#by-testtag-recommended)
- [By Text](#by-text)
- [By Content Description](#by-content-description)
- [Multiple Nodes](#multiple-nodes)
- [Hierarchical Finders](#hierarchical-finders)
- [Actions](#actions)
- [Click and Touch](#click-and-touch)
- [Text Input](#text-input)
- [Scrolling](#scrolling)
- [Gestures](#gestures)
- [Assertions](#assertions)
- [Existence and Visibility](#existence-and-visibility)
...
- [IdlingResource Integration](#idlingresource-integration)
- [Testing Patterns](#testing-patterns)
- [Test in Isolation](#test-in-isolation)
- [Test with ViewModel](#test-with-viewmodel)
- [Test Navigation](#test-navigation)
- [Test State Restoration](#test-state-restoration)
- [LazyColumn/LazyRow Testing](#lazycolumnlazyrow-testing)
- [Scroll to Item](#scroll-to-item)
- [Test Dynamic Content](#test-dynamic-content)
- [Screenshot Testing](#screenshot-testing)
- [Basic Screenshot](#basic-screenshot)
- [With Paparazzi (JVM)](#with-paparazzi-jvm)
- [Debugging](#debugging)
- [Print Semantics Tree](#print-semantics-tree)
- [Use Unmerged Tree](#use-unmerged-tree)
...
>
> Compose merges the semantics of child nodes into a single parent node by default. For example,
> a `Button` that wraps an `Icon` and a `Text` presents as one merged node whose semantics contain
> both the icon's content description and the text — the individual child nodes are invisible to
> `onNodeWithTag` or `onNodeWithText` unless you opt out.
>
> **Symptom**: `assertExists()` or `assertTextEquals()` throws "No nodes found" even though the
> element is clearly rendered, or `onAllNodesWithText("Save")` returns zero matches when the
> `Text("Save")` is a child of a `Button`.
>
> **Why it happens**: `Modifier.semantics(mergeDescendants = true)` (applied automatically by
> Material components such as `Button`, `Card`, `ListItem`, and `IconButton`) collapses the
> subtree into its root node. The merged tree is what the test framework sees by default.
>
> **Fix**: Pass `useUnmergedTree = true` to the finder when you need to assert on an individual
> child node inside a merged component:
>
> ```kotlin
> // Asserting on the Text child inside a merged Button — fails with default merged tree
> composeTestRule.onNodeWithText("Save").assertExists() // may throw: No nodes found
>
> // Pass useUnmergedTree = true to traverse the unmerged tree
> composeTestRule.onNodeWithText("Save", useUnmergedTree = true).assertExists()
>
> // Or use the tag on the parent and assert aggregated semantics
...
> possible. Drop into `useUnmergedTree = true` only when you specifically need to assert on a
> child node that the merge hides. Print both trees to compare them during investigation:
>
> ```kotlin
