...
# Target the interpreter version your CI actually runs. Python 3.14 is current stable
# (released Oct 2025); 3.12/3.13 remain in security support. Update this to match your
# runtime rather than copying the value below verbatim.
[mypy]
python_version = 3.13
strict = True
...
Prefer a modern Python toolchain for new backends:

- `uv` for dependency management, lockfiles, virtualenvs, and script execution
- `ruff` for linting and formatting
- `pytest` for tests
- `mypy` only where the team will actually keep strictness green
- Python 3.14 ships free-threaded builds (`3.14t`) as officially supported per PEP 779, with
  single-thread overhead down to roughly 5-10%. Free-threading is opt-in (a separate build), not
  yet the default interpreter — do not assume the GIL is gone unless the project deliberately
  targets `3.14t` and its C-extension dependencies (asyncpg, psycopg, etc.) have confirmed support.
