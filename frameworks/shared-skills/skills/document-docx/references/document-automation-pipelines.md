...
- [Data Sources and Batch Generation](#data-sources-and-batch-generation)
...
```text
  ...
  Renderer (docxtpl / python-docx)
       ↓
  Quality Gates (size, parse, unresolved vars, OOXML signals)
       ↓
...
Practical rule:
- Prefer `.docx` / `.dotx` templates.
- Treat `.docm` / `.dotm` as exceptional and untrusted unless the workflow explicitly requires signed macros.
...
Checklist: file exists and size > 0, within size budget, parses without error, no unresolved `{{ variables }}`, tracked-change/comment signals are surfaced, PDF conversion succeeds if required.

For local automation in this skill, prefer:

```bash
python3 scripts/docx_quality_gate.py output.docx --json
```
...
| Review metadata lost | Revision-heavy source edited with high-level library | Inspect OOXML first; switch workflow if tracked changes/comments matter |
| Macro risk in template | `.docm` / `.dotm` used casually in automation | Block or isolate macro-enabled files |
...
- [review-comments-workflows.md](review-comments-workflows.md) - Review-safe editing strategy
