...
Expert judgment for when custom OOXML manipulation (`OxmlElement`, direct `.xml` edits) goes wrong in ways a novice will not anticipate:

- **Element order is schema-enforced.** Children of `w:rPr`, `w:pPr`, and similar containers must appear in the sequence defined by the OOXML schema (e.g. `w:b` before `w:i` before `w:color` in a given `rPr`), not the order convenient for the code. Appending elements in the wrong order is a leading cause of Word's "we found a problem with some content" repair prompt on files that otherwise open fine in `python-docx`/LibreOffice (schema validation is stricter in Word than in most third-party readers).
- **`add_style`/style edits do not validate the `basedOn` chain.** Creating a custom style that is `basedOn` a style that does not exist (or was renamed) produces a file that opens in Word but silently falls back to Normal — verify the base style name exists in `doc.styles` before assigning it.
- **Re-saving a document you did not create strips authoring metadata `python-docx` does not model.** Opening a Word-authored file, making a small edit, and saving through `python-docx` is generally safe for content it understands (paragraphs, runs, tables, styles), but any XML `python-docx` has no model for (custom XML parts, some content controls, certain SmartArt/OLE structures) rides through unless something in the save path touches it — do not assume python-docx is a byte-for-byte round-trip tool once you've touched a document with these features. Diff the zip's part list (`docx_inspect_ooxml.py --list-parts`) before and after if a round trip must be lossless.
- **Fields (TOC, PAGE, REF) are opaque three-part structures** (`fldChar begin` / `instrText` / `fldChar end` or `separate`/`end`). Editing surrounding runs without touching the field frequently leaves a field with a stale cached result that only updates when a user manually refreshes it in Word, or a script runs LibreOffice with `--convert-to` (which does update fields on conversion, unlike a plain copy).
- **Direct formatting bloat**: applying `run.font.*` per run instead of a paragraph/character style is the most common reason a generated document "looks right" but fails a design/brand review — the visual result matches but the semantic structure (used by accessibility tools, TOC generation, and downstream Markdown/HTML extraction) does not.

## Related Resources

- [SKILL.md](../SKILL.md) - Quick reference
- [template-workflows.md](template-workflows.md) - Batch generation patterns
- [review-comments-workflows.md](review-comments-workflows.md) - Comments and Word Compare
- [llm-extraction-workflows.md](llm-extraction-workflows.md) - Extraction-oriented DOCX workflows
