...
- [Contents](#contents)
- [CI/CD Quality Gate Checklist](#cicd-quality-gate-checklist)
- [GitHub Actions Quality Gates](#github-actions-quality-gates)
- [GitLab Merge Trains](#gitlab-merge-trains)
- [GitLab CI Quality Gates](#gitlab-ci-quality-gates)
- [Pre-commit Hooks (Local Quality Gates)](#pre-commit-hooks-local-quality-gates)
- [Quality Metrics & Thresholds](#quality-metrics--thresholds)
- [PR Status Checks (GitHub)](#pr-status-checks-github)
- [Automated PR Comments](#automated-pr-comments)
- [Skipping Quality Gates (Emergency Hotfixes)](#skipping-quality-gates-emergency-hotfixes)
- [Quality Gate Exemptions](#quality-gate-exemptions)
- [Monitoring Quality Trends](#monitoring-quality-trends)
...
When configuring merge queue on GitHub, prefer doing it through **Rules -> Rulesets** so queue requirements, required checks, and branch targeting live in one governance layer.
...
For GitLab, pair merge request pipelines with **merge trains** when many branches compete to land on the default branch.

**Use merge trains when**:

- The target branch must stay green under heavy concurrency
- Re-running full pipelines after each merge is expensive
- You want queued MRs to be validated in merge order before landing

**Operational guidance**:

- Keep MR pipelines stable before enabling merge trains
- Use CODEOWNERS and approval rules before the train stage
- Watch queue time, flaky tests, and canceled pipelines

See [GitLab merge trains documentation](https://docs.gitlab.com/ci/pipelines/merge_trains/) for configuration details and limits.
