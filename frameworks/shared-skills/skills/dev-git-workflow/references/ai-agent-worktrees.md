...
Git worktrees enable AI coding agents (Claude Code, Codex, Aider, and similar agents) to work in isolated directories while sharing the same repository. Each agent gets its own checkout, its own branch, and its own working tree — no `git stash` juggling, no accidental cross-contamination.

## Table of Contents

- [Typical Flow](#typical-flow)
- [Parallel Multi-Agent Detail](#parallel-multi-agent-detail)
- [When to Use Worktrees with AI Agents](#when-to-use-worktrees-with-ai-agents)
- [Directory Conventions](#directory-conventions)
- [1. Project-local hidden directory (preferred)](#1-project-local-hidden-directory-preferred)
- [2. Project-local visible directory](#2-project-local-visible-directory)
- [3. Global directory (outside project)](#3-global-directory-outside-project)
- [Priority resolution](#priority-resolution)
- [Setup: One Worktree per Agent](#setup-one-worktree-per-agent)
- [Step 1: Verify `.gitignore` (project-local only)](#step-1-verify-gitignore-project-local-only)
- [Check if worktree directory is ignored](#check-if-worktree-directory-is-ignored)
- [Step 2: Create worktree with feature branch](#step-2-create-worktree-with-feature-branch)
- [From main checkout](#from-main-checkout)
- [Or from an existing remote branch](#or-from-an-existing-remote-branch)
- [Step 3: Install dependencies (auto-detect)](#step-3-install-dependencies-auto-detect)
- [Node.js](#nodejs)
- [Python](#python)
- [Rust](#rust)
- [Go](#go)
- [Step 4: Verify clean baseline](#step-4-verify-clean-baseline)
- [Run project tests to confirm worktree starts clean](#run-project-tests-to-confirm-worktree-starts-clean)
- [Agent-Specific Patterns](#agent-specific-patterns)
- [Claude Code](#claude-code)
- [Launch agent 1 in its own worktree](#launch-agent-1-in-its-own-worktree)
- [Launch agent 2 in a separate worktree (parallel terminal)](#launch-agent-2-in-a-separate-worktree-parallel-terminal)
- [OpenAI Codex](#openai-codex)
- [Create a worktree to review/test Codex's branch locally](#create-a-worktree-to-reviewtest-codexs-branch-locally)
- [Aider / Other Terminal Agents](#aider-other-terminal-agents)
- [Create worktree, then point the agent at it](#create-worktree-then-point-the-agent-at-it)
- [Other Cloud Agents](#other-cloud-agents)
- [Parallel Agent Execution](#parallel-agent-execution)
- [Architecture](#architecture)
- [Rules for parallel agents](#rules-for-parallel-agents)
- [Launching parallel agents (shell example)](#launching-parallel-agents-shell-example)
- [Create worktrees](#create-worktrees)
- [Launch agents in parallel (background processes)](#launch-agents-in-parallel-background-processes)
- [Orchestrator reviews from main checkout](#orchestrator-reviews-from-main-checkout)
- [Merge strategy after parallel work](#merge-strategy-after-parallel-work)
- [Safety and Conflict Prevention](#safety-and-conflict-prevention)
- [`.gitignore` discipline](#gitignore-discipline)
- [Lock file contention](#lock-file-contention)
- [Preventing cross-agent file conflicts](#preventing-cross-agent-file-conflicts)
- [Check if branches touch the same files](#check-if-branches-touch-the-same-files)
- [If output is non-empty, review those files for conflicts](#if-output-is-non-empty-review-those-files-for-conflicts)
- [Stale worktree detection](#stale-worktree-detection)
- [List all worktrees and their branch status](#list-all-worktrees-and-their-branch-status)
- [Find worktrees with branches already merged to main](#find-worktrees-with-branches-already-merged-to-main)
- [Cleanup Lifecycle](#cleanup-lifecycle)
- [Standard cleanup](#standard-cleanup)
- [1. Remove the worktree](#1-remove-the-worktree)
- [2. Delete the branch (if merged)](#2-delete-the-branch-if-merged)
- [3. Prune worktree metadata (if worktree was deleted manually)](#3-prune-worktree-metadata-if-worktree-was-deleted-manually)
- [Batch cleanup script](#batch-cleanup-script)
- [Clean up all worktrees whose branches are merged to main](#clean-up-all-worktrees-whose-branches-are-merged-to-main)
...
- [Create worktree from existing remote branch](#create-worktree-from-existing-remote-branch)
- [List all worktrees](#list-all-worktrees)
- [Remove a worktree](#remove-a-worktree)
- [Prune stale worktree metadata](#prune-stale-worktree-metadata)
- [Check if directory is gitignored](#check-if-directory-is-gitignored)
- [See which files two branches both changed](#see-which-files-two-branches-both-changed)
- [Move a worktree to a new location](#move-a-worktree-to-a-new-location)
...
Claude Code ships native worktree isolation as of early 2026. The worktree model is the standard isolation primitive for parallel agents — one task, one branch, one worktree, one agent.
...
Cloud-based agents already run in isolated environments. Use worktrees when you need to review, test, or merge their output locally.
...
- `agents-subagents` — Orchestration, handoffs, wave dispatch
