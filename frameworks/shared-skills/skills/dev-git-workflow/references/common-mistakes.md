...
- [Contents](#contents)
- [Mistake 1: Large, Unfocused PRs](#mistake-1-large-unfocused-prs)
- [Mistake 2: Vague Commit Messages](#mistake-2-vague-commit-messages)
- [Mistake 3: Rewriting Public History](#mistake-3-rewriting-public-history)
- [Mistake 4: Ignoring Review Comments](#mistake-4-ignoring-review-comments)
- [Mistake 5: Not Pulling Before Starting Work](#mistake-5-not-pulling-before-starting-work)
- [Mistake 6: Committing Secrets](#mistake-6-committing-secrets)
- [Mistake 7: Mixing Refactoring with Features](#mistake-7-mixing-refactoring-with-features)
- [Mistake 8: Not Testing Before Pushing](#mistake-8-not-testing-before-pushing)
- [Mistake 9: Unclear PR Descriptions](#mistake-9-unclear-pr-descriptions)
- [What](#what)
- [Why](#why)
- [How](#how)
- [Testing](#testing)
...
- [Related](#related)
- [Mistake 10: Force Push Without `--force-with-lease`](#mistake-10-force-push-without---force-with-lease)
...
```bash
# 1. Rotate the compromised credentials immediately — this closes the actual
#    exposure and is required regardless of whether you also rewrite history.

# 2. Only if the leaked file must also be scrubbed from history, coordinate
#    with the team first, then use git filter-repo (the Git project's own
#    docs deprecate filter-branch as unsafe and unusably slow on non-trivial
#    repos; filter-repo is the documented replacement):
git filter-repo --path config/secrets.json --invert-paths

# Or use BFG Repo-Cleaner (simpler for large binary/secret cleanup):
java -jar bfg.jar --delete-files secrets.json
git reflog expire --expire=now --all && git gc --prune=now --aggressive

# 3. Force push only after the team has been notified everyone must re-clone
