...
- [Formative vs Summative Testing](#formative-vs-summative-testing)
- [Test Planning](#test-planning)
- [Objectives Definition](#objectives-definition)
- [Participant Recruitment](#participant-recruitment)
- [Screener Questions](#screener-questions)
- [Task Design](#task-design)
- [Task Scenario Writing](#task-scenario-writing)
- [Task Complexity Balancing](#task-complexity-balancing)
- [Red Routes Prioritization](#red-routes-prioritization)
- [Think-Aloud Protocol](#think-aloud-protocol)
- [Types of Think-Aloud](#types-of-think-aloud)
- [Facilitator Introduction Script](#facilitator-introduction-script)
- [Probing Questions](#probing-questions)
- [Avoiding Bias](#avoiding-bias)
- [Data Collection](#data-collection)
- [Task-Level Metrics](#task-level-metrics)
- [Task Success Scoring](#task-success-scoring)
...
- [Affinity Mapping Process](#affinity-mapping-process)
- [Issue Severity Assignment](#issue-severity-assignment)
- [Pattern Identification](#pattern-identification)
- [Statistical Significance (Quantitative)](#statistical-significance-quantitative)
- [Reporting Formats](#reporting-formats)
- [Highlight Reel](#highlight-reel)
- [Executive Summary](#executive-summary)
- [Detailed Findings Report](#detailed-findings-report)
- [Remote Testing Tools](#remote-testing-tools)
- [Tool Comparison](#tool-comparison)
- [Tool Selection Criteria](#tool-selection-criteria)
- [DIY Remote Testing Setup](#diy-remote-testing-setup)
...
|-----------|-------------------|-----|
| Qualitative (find issues) | 5-8 per segment | ~85% of usability problems surfaced with 5 users (directional — see below) |
| Quantitative (measure) | 20-50 per variant | Statistical significance |
| A/B testing | 1000+ per variant | Detect small differences |

**On the "5 users, 85%" figure**: this comes from Nielsen and Landauer's model (Nielsen & Landauer, 1993; popularized in Nielsen's "Why You Only Need to Test with 5 Users," NN/g), not a universal law. The model is `ProblemsFound(n) = N × (1 − (1 − λ)ⁿ)`, where λ is the average per-user probability of detecting any given problem. Nielsen and Landauer measured λ ≈ 0.31 averaged across the projects they studied — at that value, 5 users surface ~85% of the *detectable* problems in that specific test. The figure depends on three assumptions that often don't hold: (1) a single, fairly homogeneous target-user group — mixed segments need 5 users *per segment*, not 5 total; (2) λ itself varies a lot by product and study (a low-λ study needs far more than 5 users for the same coverage); (3) it describes an *iterative* testing context — run small rounds across design iterations rather than treating one 5-user round as final sign-off. Treat "5 users → 85%" as a planning heuristic for formative testing, not a statistical guarantee, and do not apply it to summative/benchmark studies (see Formative vs Summative Testing above).
...
|------|------|----------|-------------|
| **UserTesting** (incl. former UserZoom; merged 2023) | Moderated + unmoderated, panel | Large-scale studies, video feedback, enterprise | $$$ |
| **Maze** | Unmoderated, AI moderation | Rapid prototype testing | $$ |
| **Lyssna** (formerly UsabilityHub) | Unmoderated | First-click, preference, quick tests | $ |
| **Lookback** | Moderated | Live sessions, recording | $$ |
| **Great Question** | Research ops + moderated | Repository + live sessions in one stack | $$ |
| **Sprig** | In-product micro-studies | In-context surveys and replays at signup/activation | $$ |
| **dscout** | Diary studies + moderated | Longitudinal and in-context mobile research | $$$ |
| **Optimal Workshop** | IA testing | Card sorts, tree tests | $$ |
| **Hotjar / FullStory / Pendo** | Behavior analytics | Heatmaps, session recordings, replay-to-research bridging | $ – $$$ |
| **Zoom / Google Meet** | Moderated | Live sessions (DIY) | $ |

Note: UserTesting and UserZoom completed their merger in 2023 and bill as a single platform. Treat them as one product.
