...
**Freshness anchor:** 2026-05 — use current provider-native multimodal families and structured-output features, then re-verify exact model names before rollout.

### Claude Opus 4.7: high-resolution image support

Claude Opus 4.7 (`claude-opus-4-7`) raises the maximum image resolution to **2576px / 3.75MP** (up from 1568px / 1.15MP on prior models). Coordinates returned by the model are **1:1 with actual pixels** — no scale-factor math is required when mapping model-returned coordinates back to image pixels. This simplifies bounding-box workflows and UI automation (see Pattern 3 and Pattern 5 below).

High-res images use more tokens. If the additional fidelity is not needed for your task, downsample before sending to control cost.
...
- [Extraction vs Interpretation](#extraction-vs-interpretation)
- [Vision Prompt Patterns](#vision-prompt-patterns)
- [Pattern 1: General Image Description](#pattern-1-general-image-description)
- [Pattern 2: Structured Data Extraction](#pattern-2-structured-data-extraction)
- [Pattern 3: Bounding Box / Region Identification](#pattern-3-bounding-box-region-identification)
- [Pattern 4: OCR + Understanding](#pattern-4-ocr-understanding)
- [Pattern 5: Set-of-Marks for UI](#pattern-5-set-of-marks-for-ui)
- [Pattern 6: Multi-Image Comparison](#pattern-6-multi-image-comparison)
- [Document Understanding Prompts](#document-understanding-prompts)
- [Pattern 7: Table Extraction](#pattern-7-table-extraction)
- [Pattern 8: Form Understanding](#pattern-8-form-understanding)
- [Pattern 9: Multi-Page Document Processing](#pattern-9-multi-page-document-processing)
- [Audio Prompt Patterns](#audio-prompt-patterns)
- [Pattern 10: Whisper Prompt Conditioning](#pattern-10-whisper-prompt-conditioning)
- [Whisper API with prompt conditioning](#whisper-api-with-prompt-conditioning)
- [Pattern 11: Transcript Post-Processing](#pattern-11-transcript-post-processing)
- [Pattern 12: Speaker Diarization Post-Processing](#pattern-12-speaker-diarization-post-processing)
- [Video Frame Analysis](#video-frame-analysis)
- [Frame Sampling Strategy](#frame-sampling-strategy)
- [Video Analysis Prompt](#video-analysis-prompt)
- [Frame Extraction Notes](#frame-extraction-notes)
- [Prompt Engineering Tips by Modality](#prompt-engineering-tips-by-modality)
- [Vision-Specific Tips](#vision-specific-tips)
- [Audio-Specific Tips](#audio-specific-tips)
- [Document-Specific Tips](#document-specific-tips)
...
|---|---|---|---|
| Describe an image | Free-form interpretation | Any current vision model | $ |
| Extract text from image | OCR prompt | OCR-optimized multimodal model | $ |
| Extract structured data from image | Schema-first extraction | Structured-output-capable multimodal model | $$ |
| Compare two images | Side-by-side comparison | Current high-quality multimodal model | $$ |
| Analyze UI screenshot | Set-of-marks + action | Current multimodal reasoning model | $$ |
| Transcribe audio | STT + formatting prompt | Current speech model + LLM | $ |
| Extract data from PDF | Schema-first document extraction | Document-capable multimodal model | $$ |
| Parse tables from images | Table schema prompt | Structured-output-capable multimodal model | $$ |
| Analyze video | Frame sampling + vision | Native video model or frame pipeline | $$$ |
| Identify objects with locations | Bounding box prompt | Region-capable vision model | $$ |

### Extraction vs Interpretation

- Use **schema-first extraction** for OCR, forms, tables, receipts, and contracts.
- Use **free-form interpretation** for descriptions, scene understanding, and visual comparisons.
- Evaluate extraction and interpretation separately. A model can be good at one and weak at the other.
...
- Resize frames to 1024px wide (16:9) for cost efficiency when high-res is not needed; for Claude Opus 4.7 tasks requiring pixel-accurate localization, use native resolution up to 2576px
- Encode as JPEG 85% quality, base64 for API submission
- For providers with native video support: pass the video directly when cost and latency are acceptable (verify current model capabilities against provider docs — cross-model video support varies)
...
| Use private reasoning or provider-native thinking controls for complex images | Improves analysis without exposing scratchpads | "Validate the scene carefully and return only the final answer." |
