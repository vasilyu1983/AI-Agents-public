...
- Symptom: migrator behavior differs from the repository's established test fixture or deployment path.
  - Cause: custom command flags (for example `--readycheck`) diverge from baseline.
  - Fix: use the repository's existing migrator command and mounted migration path unless the test explicitly owns a documented override.
