...
if [[ "$NODE_MAJOR" -lt "$MIN_NODE_MAJOR" ]] || { [[ "$NODE_MAJOR" -eq "$MIN_NODE_MAJOR" ]] && [[ "$NODE_MINOR" -lt "$MIN_NODE_MINOR" ]]; }; then
  fail "Runtime preflight failed: node v${NODE_VERSION_RAW} detected, requires >= v${MIN_NODE_MAJOR}.${MIN_NODE_MINOR}.0. Run: nvm install ${MIN_NODE_MAJOR}.${MIN_NODE_MINOR}.0 && nvm use ${MIN_NODE_MAJOR}.${MIN_NODE_MINOR}.0"
fi
