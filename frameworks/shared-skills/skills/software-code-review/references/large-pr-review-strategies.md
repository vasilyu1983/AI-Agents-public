...
| PR Size (Lines Changed) | Category | Typical Review Time | Reviewer Effort |
|--------------------------|----------|-------------|----------------------|
| 1-50 | Small | 5-10 min | Full attention, single pass |
| 51-200 | Medium | 15-30 min | Full attention, single pass |
| 201-400 | Large | 30-90 min | Multi-pass; approaching the range where quality starts to slip |
| 401-1000 | Very Large | 60-120+ min | Split if possible; partition and delegate if not |
| 1000+ | Unmanageable | 2+ hours | Request a split unless it is a migration or generated diff |

The specific "Review Time" and "Reviewer Effort" columns are qualitative guidance, not measured rates — treat them as a starting heuristic to recalibrate against your own team's data. The one number with a real empirical source is the **200-400 LOC** review-size threshold below.
...
The most-cited empirical data point traces to Jason Cohen's Cisco Systems code-review study (published as *Best Kept Secrets of Peer Code Review*, SmartBear, 2006) — one company's dataset, not a universal law, but a well-evidenced starting point:

- **Review effectiveness drops once a single review exceeds roughly 200-400 lines.** The study found reviewers pacing faster than ~400-450 LOC/hour caught fewer defects; treat "well under 400-450 LOC/hour" as the signal, not a hard cutoff.
- **The study's own reviews ran roughly 60-90 minutes.** Beyond that, reviewer fatigue is a well-documented risk; break up longer reviews rather than pushing through in one sitting.

Google's and Microsoft's own engineering-practices guides ([Google Engineering Practices](https://google.github.io/eng-practices/review/developer/small-cls.html), [Microsoft Code With Engineering Playbook](https://microsoft.github.io/code-with-engineering-playbook/code-reviews/)) independently recommend small, single-purpose changes, but publish process guidance rather than the same kind of measured defect-rate dataset — cite them for the "keep it small" recommendation, not for a specific LOC/hour number.

Directionally consistent, but not independently re-measured here:

- **Review speed tends to decline non-linearly with size.** Context-switching overhead means a 400-line PR often takes more than 2x as long to review as a 200-line PR, not just proportionally longer.
- **Defect escape rate tends to correlate with size.** Large PRs are more often merged with undetected issues because reviewers experience fatigue and start skimming.
- **Merge conflict probability tends to rise** with the number of files touched and the time a branch stays open, though the "quadratic" shape is illustrative rather than measured.
...
Set a timer. If a PR requires more than roughly 60-90 minutes of focused review, stop and request that the author split it. Continuing well past that yields diminishing returns as fatigue sets in.
...
### The 200-400 Line Effect

Working-memory research (Miller's classic "seven plus or minus two," later narrowed by Cowan to roughly four chunks for unaided recall) is often invoked to explain why review quality degrades with size, but treat that link as an illustrative analogy, not a measured cause. The concrete, sourced signal is the Cisco/SmartBear finding above: past roughly 200-400 lines, a reviewer's mental model of the change starts degrading and defect-finding drops off.
...
- Set a per-PR time limit (roughly 60-90 minutes; see the Cisco/SmartBear pacing data above)
...
- [ ] Time-box the review session (roughly 60-90 minutes)
...
- [automation-tools.md](automation-tools.md) -- CI/CD automation, native merge queues, and AI review controls
- [deterministic-vs-llm-routing.md](deterministic-vs-llm-routing.md) -- A related but distinct size threshold: gating whether an *AI reviewer* runs an extra planning pass before reviewing a file, as opposed to this file's 200-400 LOC guidance on when a *human* reviewer's pace looks suspicious for the diff size

Source for the size/pace/timebox figures: Jason Cohen's Cisco Systems code-review study, published as *Best Kept Secrets of Peer Code Review* (SmartBear, 2006); summarized in [Smart Bear, Cisco, and the Largest Study on Code Review Ever](https://mikeconley.ca/blog/2009/09/14/smart-bear-cisco-and-the-largest-study-on-code-review-ever/) and [SmartBear — Best Practices for Peer Code Review](https://smartbear.com/learn/code-review/best-practices-for-peer-code-review/). Treat it as a well-evidenced heuristic from one company's data, not a universal law.
