...
2. **A `Task { ... }` closure formed in a nonisolated delegate/completion context mutates an `@Observable` property** — `Task` inherits the actor context where its closure is formed. A task created inside a main-actor-isolated function inherits the main actor; a task created in a nonisolated callback does not. Fix the boundary with `Task { @MainActor [weak self] in ... }`, and do not misdiagnose a main-actor task as detached. See [swiftui-observation-concurrency.md → `Task { }` inherits its actor context](../../software-ios-native/references/swiftui-observation-concurrency.md#task--inherits-its-actor-context).
