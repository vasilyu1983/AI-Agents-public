...
**CCE vs. CE and regret type**: Vanishing external regret yields empirical play approaching the coarse correlated equilibrium (CCE) set. Vanishing internal/swap regret yields correlated equilibrium (CE); Hart and Mas-Colell's (2000) specific regret-matching procedure guarantees empirical distributions converge to the CE set with probability one. Do not conflate these guarantees, claim last-iterate convergence, or infer that CCE is the only computationally feasible target. Explicit finite normal-form CE can be computed through a linear program; representation and game size determine practical cost. [Primary Hart/Mas-Colell abstract](https://doi.org/10.1111/1468-0262.00153), rechecked 2026-09-17.
...
| Bilateral-trade impossibility | Single buyer/seller, independent private values with overlapping support, Bayesian incentive compatibility | Under the standard independent private-value model with overlapping support, no mechanism jointly satisfies Bayesian incentive compatibility, individual rationality, ex-post efficiency and budget balance (Myerson & Satterthwaite 1983). Dropping incentive compatibility removes this particular impossibility; retaining the conditions forces a tradeoff, not a fixable implementation bug |
