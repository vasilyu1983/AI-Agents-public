...
- **Benchmark-chasing.** Teams sometimes discover that a specific benchmark's task distribution has quirks (certain phrasing patterns, certain tool-call shapes) that can be gamed without improving the underlying capability. See `agentic-benchmarks.md` for the published, ABC-documented version of this at the public-benchmark level (SWE-bench Verified, TAU-bench).
...
# n = 906 samples needed per condition (re-derived 2026-07-11; verify with your own
# stats library before using — do not trust unverified sample-size comments in code)
```
...
**These are rough floors, not a substitute for the power calculation above.** The worked example (85% baseline, detecting a 5-point drop, alpha 0.05, power 0.80) requires 906 per group — well above the 350+ floor in this table. As baseline accuracy approaches 90-95% or the effect you need to detect shrinks below ~5 points, required N grows fast and can run into the thousands. Run `required_sample_size()` (or an equivalent power calculation) with your own baseline and minimum detectable effect before committing to a dataset size for a real statistical comparison — do not just read a number off this table.
