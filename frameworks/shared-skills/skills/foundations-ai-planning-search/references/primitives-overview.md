...
**Cost-shaping the orchestrator**: when a verifier-guided repair loop is already producing certified-correct trajectories, those trajectories are supervision. Distilling the orchestration policy — which repair action to apply in which state — into a small fine-tuned model has been reported to match frontier-model orchestration quality on PDDL repair at approximately 45x lower orchestration cost than GPT-5-mini and 15x lower than Gemini-3-Flash per task and 40–50 % fewer LLM calls (Mangannavar, Coalson, Dugar & Tadepalli, arXiv:2606.21740, 2026). Treat this as a maturity-stage optimization, not a starting architecture: it presupposes you already have a sound verifier and enough accepted trajectories to train on.
