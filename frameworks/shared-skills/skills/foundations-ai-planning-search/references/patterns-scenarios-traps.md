...
- [Validity Gate Vs Safety Gate](#validity-gate-vs-safety-gate)
...
- Domain generated by an LLM is checked for syntax but never for semantics — it parses, and it means something other than the description said.
- The validity gate is treated as the safety gate. Preconditions and effects answer "can this action run here," not "should it." Dangerous-but-legal action sequences pass a validator cleanly.
...
A third axis applies once an LLM is in the loop: **who owns the formal model**. If the LLM writes the PDDL domain and a sound planner solves it, errors concentrate in the domain model — inspectable once, testable against known instances, fixed permanently. If the LLM emits plans directly, errors are per-request and must be caught every time. Prefer the first arrangement wherever a planner can be invoked at all.

## Validity Gate Vs Safety Gate

These are two different gates and conflating them is a design error, not a tuning problem.

- **Validity** asks: do this action's preconditions hold in the current state, and does the effect model correctly produce the next state? This is what a PDDL validator, a typed function-call schema, or a precondition replay checks. It is a property of the domain model.
- **Safety** asks: should this action, or this goal, be executed at all — given irreversibility, blast radius, spend, physical risk, policy, or permission? This is a property of the action *and its context*, not of the transition model.

A plan can be perfectly valid and entirely unsafe, and the cited benchmark shows that improving validity does not reliably improve danger avoidance: measured separately on embodied planning tasks, near-perfect plan validity coexisted with dangerous plans on roughly a quarter of tasks, and scaling model size raised validity dramatically while leaving danger-detection roughly flat (see the DESPITE result in `primitives-overview.md` #7). Consequences for design:

1. Do not let the planner's success metric stand in for a safety metric in any dashboard, eval, or ship criterion. Track them as separate numbers.
2. Implement the safety gate as a distinct component over the action set — an allowlist, a permission check, a human-approval trigger for irreversible actions — that runs after validity and can veto a valid plan.
3. Test the safety gate with cases that are deliberately valid-but-dangerous. A test suite built from invalid plans exercises the wrong gate entirely.
4. Improving the planner is not a safety roadmap. If danger detection is the gap, it needs its own work item.
