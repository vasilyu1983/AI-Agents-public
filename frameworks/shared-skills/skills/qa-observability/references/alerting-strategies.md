...
groups:
  - name: slo-burn-rate-alerts
    rules:
      ...
      # P3 (ticket): 10% of budget in 1 day
      - alert: SLOBurnRateTicket
        expr: |
          (
            (1 - rate(http_requests_total{status!~"5.."}[1d])
             / rate(http_requests_total[1d]))
            > (3 * 0.001)
          )
          and
          (
            (1 - rate(http_requests_total{status!~"5.."}[2h])
        ...
        labels:
          ...
          team: "{{ $labels.team }}"
        annotations:
          summary: "Elevated SLO burn rate for {{ $labels.service }} (ticket, not page)"
          runbook: "https://runbooks.example.com/slo-burn-ticket"

      # P4: Slow burn - 10% of budget in 3 days
...
| Ticket (P3) | 1d | 2h | 3x | 10% in 1d | ~10 days |
| Slow (P4) | 3d | 6h | 1x | 10% in 3d | ~30 days |

This four-tier table matches the canonical Google SRE Workbook (2018), Chapter 5, "Alerting on SLOs" configuration and must stay consistent with the same table in [`slo-design-guide.md`](./slo-design-guide.md#burn-rate-alerts) and [`reliability-theory-applied.md`](./reliability-theory-applied.md#p2--slo-burn-rate-alerting-derived-from-error-budgets). If you edit one, edit all three.
