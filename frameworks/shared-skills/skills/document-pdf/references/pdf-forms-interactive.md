...
### Minimal pyHanko Workflow

Use pyHanko when you control the signing credentials and need a reproducible PDF-signing path:

1. Start with an AcroForm PDF and add a dedicated signature field in the template when visible signing is required.
2. Sign with pyHanko using a PKCS#12 file, hardware token, or PKCS#11-backed credential.
3. Validate the signed output in Adobe Acrobat/Reader and in your relying-party validation flow.

Keep interactive form handling and digital signatures conceptually separate. Filling an AcroForm is a content operation; signing is a cryptographic operation with certificate, timestamp, and trust-chain requirements.
