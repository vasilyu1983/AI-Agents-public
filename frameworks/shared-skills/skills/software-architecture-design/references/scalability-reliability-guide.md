...
- [Key Metrics (SRE)](#key-metrics-sre)
- [Service Level Indicator (SLI)](#service-level-indicator-sli)
- [Service Level Objective (SLO)](#service-level-objective-slo)
- [Service Level Agreement (SLA) - Customer commitment](#service-level-agreement-sla-customer-commitment)
...
- **Reality**: Partition tolerance is mandatory in distributed systems, so the meaningful tradeoff under partition is CP vs AP

### Scalability Dimensions

**Vertical scaling** (scale up):
- Add more CPU, RAM, storage to single machine
- Limits: Hardware maximums (cloud high-memory instances now reach tens of TB RAM and 400+ vCPUs, e.g. AWS u-series; verify current ceilings per provider before assuming a scale-up wall)
