...
Deep reference for API gateway architectures, service mesh implementation (Istio, Linkerd, Envoy, Cilium), sidecar and ambient patterns, and service-to-service communication. Use when designing inter-service communication, implementing traffic management, or choosing between gateway, mesh, or hybrid topologies.
...
A service mesh is a dedicated infrastructure layer for service-to-service communication. It can be implemented with sidecar proxies, ambient/sidecar-less data planes, or eBPF-assisted networking depending on the platform and control requirements.
...
### Mesh Data Plane Responsibilities
...
| Mode | Strengths | Trade-offs | Use When |
|------|-----------|------------|----------|
| Sidecar mesh | Mature feature set, per-pod isolation, broad ecosystem support | Higher CPU/memory overhead, more moving parts | Advanced traffic policy, mature mesh operations |
| Ambient / sidecar-less mesh | Lower per-pod overhead, simpler app pods, easier gradual adoption | Feature maturity varies by platform, different debugging model | Large clusters, cost-sensitive platforms, teams avoiding sidecar tax |
| No mesh | Lowest complexity, no extra infra layer | Less centralized policy and telemetry | Small systems, stable library patterns, limited traffic policy needs |
...
|---------|-------|---------|--------------|---------------------|
| Data plane | Envoy sidecar or ambient mode | linkerd2-proxy (Rust) | Envoy | eBPF + Envoy where needed |
| Complexity | High | Low | Medium | Medium |
| Resource overhead | Medium-High (lower with ambient than sidecars) | Lower (lightweight proxy) | Medium | Lowest (kernel-level) |
...
| Best for | Complex mesh, advanced traffic, ambient adoption | Simple mesh, low overhead | Existing AWS App Mesh estates only | High performance, eBPF-aware platforms |
...
Service mesh selection:
  ...
  ├─ 10-50 services, want simplicity → Linkerd or no mesh
  ├─ 10-50 services, need advanced traffic mgmt → Istio (consider ambient first)
  ├─ Existing AWS App Mesh estate → Plan migration (AWS-native: VPC Lattice; portable: Istio/Cilium); avoid for net-new
  ├─ Performance-critical, eBPF available → Cilium
  └─ > 50 services, multi-cluster → Istio or Cilium, chosen by platform constraints
```

Note: AWS has announced end of support for AWS App Mesh on September 30, 2026, so do not choose it for new architectures. For existing estates, the AWS-native migration target is Amazon VPC Lattice; choose Istio or Cilium where cross-cloud portability matters. Verify the current EoL date and migration guidance against AWS's official announcement before planning a cutover.
