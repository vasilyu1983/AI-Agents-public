...
- [1. Canonical Coding Principles](#1-canonical-coding-principles)
...
- [4.1 When to Refactor](#41-when-to-refactor)
- [4.2 Refactor vs Rewrite](#42-refactor-vs-rewrite)
- [4.3 Refactor Scope](#43-refactor-scope)
- [4.4 Choosing Refactor Operations](#44-choosing-refactor-operations)
- [5. Legacy Code Survival Kit](#5-legacy-code-survival-kit)
- [5.1 Stabilize First](#51-stabilize-first)
- [5.2 Work at Seams](#52-work-at-seams)
- [5.3 Incremental Change](#53-incremental-change)
- [5.4 Knowledge Capture](#54-knowledge-capture)
- [5.5 Dependency De-risking](#55-dependency-de-risking)
- [5.6 Retirement and Cleanup](#56-retirement-and-cleanup)
...
- [7.1 God Object / God Service](#71-god-object-god-service)
- [7.2 Long Method](#72-long-method)
- [7.3 Primitive Obsession](#73-primitive-obsession)
- [7.4 Shotgun Surgery](#74-shotgun-surgery)
- [7.5 Feature Envy](#75-feature-envy)
- [7.6 Global Mutable State](#76-global-mutable-state)
- [7.7 Commented-Out Code and Redundant Comments](#77-commented-out-code-and-redundant-comments)
...
- [10.8 Peer Review Controls (Best Kept Secrets of Peer Code Review)](#108-peer-review-controls-best-kept-secrets-of-peer-code-review)
...
- [TypeScript/JavaScript - Check if package/version exists](#typescriptjavascript-check-if-packageversion-exists)
- [Python - Check if package exists and version](#python-check-if-package-exists-and-version)
- [Verify imports resolve](#verify-imports-resolve)
- [TypeScript](#typescript)
- [Python](#python)
- [BAD: Hallucinated - pandas method doesn't exist](#bad-hallucinated-pandas-method-doesnt-exist)
- [GOOD: Correct - Use actual pandas API](#good-correct-use-actual-pandas-api)
...
- [LLM Code Review Checklist](#llm-code-review-checklist)
- [Verification (Blocking)](#verification-blocking)
- [Quality (Required)](#quality-required)
- [Documentation (Expected)](#documentation-expected)
- [Decision](#decision)
...
- [Log LLM code issues for analysis](#log-llm-code-issues-for-analysis)
...
- [GitHub Actions complexity gate](#github-actions-complexity-gate)
...
- [13.1 Debt Classification](#131-debt-classification)
- [13.2 Debt Entry Template](#132-debt-entry-template)
- [TD-[NUMBER]: [Brief Title]](#td-number-brief-title)
- [Description](#description)
- [Impact](#impact)
- [Remediation](#remediation)
- [Cost of Delay](#cost-of-delay)
- [Related](#related)
- [13.3 Debt Register Example](#133-debt-register-example)
- [13.4 Debt Tracking Workflow](#134-debt-tracking-workflow)
- [13.5 Debt Prevention Strategies](#135-debt-prevention-strategies)
- [13.6 Debt Metrics Dashboard](#136-debt-metrics-dashboard)
- [13.7 Agent-Executable Debt Management Checklist](#137-agent-executable-debt-management-checklist)
...
- Size limits: the study found review effectiveness drops once a single review exceeds roughly 200–400 LOC; keep reviews in that range and split larger changes. Reject or rescope anything far beyond that or that cannot be reviewed in one focused sitting.
- Pace: the study observed that reviewers slower than ~400 LOC/hour found more defects, and defect density was below average in most reviews faster than ~450 LOC/hour. Treat "aim for well under 400–450 LOC/hour" as the signal, not a hard cutoff.
- Timebox: the study's own reviews ran roughly 60–90 minutes; beyond that, reviewer fatigue is a well-documented risk. Break up longer reviews rather than pushing through in one sitting.
...
Source: [Smart Bear, Cisco, and the Largest Study on Code Review Ever](https://mikeconley.ca/blog/2009/09/14/smart-bear-cisco-and-the-largest-study-on-code-review-ever/) (summary of Cohen's Cisco study); [SmartBear — Best Practices for Peer Code Review](https://smartbear.com/learn/code-review/best-practices-for-peer-code-review/).
...
Targets below are illustrative placeholders for the template — replace with numbers calibrated to your own team's size, domain risk, and historical trend; they are not industry benchmarks.
...
Clean-code guidance is durable where it encodes coupling/cohesion, naming, and interface design; it becomes cargo cult when specific numeric rules (function length, comment bans, DRY-at-all-costs) are applied as universal law regardless of context. This section makes that distinction explicit so agents apply RULE-01–RULE-13 and the CC-* catalog with judgment, not by rote.
...
- **Practical takeaway**: treat CC-FUN-05 as "eliminate duplication that is bug-prone or certain to diverge together," not "never write similar-looking code twice."

### 14.3 When NOT to Refactor

Refactoring is a cost with a payoff, not a default good. Skip or defer refactoring when:

- The code is stable, low-change-frequency, and correct — improving its internals produces review risk with no user-visible benefit (see § 4.1–4.2).
- A release deadline makes a large behavior-preserving refactor itself the riskiest change in the diff — prefer the smallest change that satisfies the request and log the cleanup as debt (§ 13) instead.
- Test coverage is too weak to safely validate that behavior is preserved, and adding characterization tests is out of scope for the current change — refactor only after coverage exists (§ 5.1).
