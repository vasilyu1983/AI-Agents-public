...
| Pure functions | **CC-FUN** (Functions/methods) | Small, single-purpose, testable without mocks |
| Immutability | **CC-FUN**, **CC-TYP** | Reduce shared mutable state; safer concurrency |
| Pipe/Compose | **CC-FUN** | Composable functions with clear data flow |
| Discriminated unions | **CC-TYP** (Types & data structures) | Model domain states precisely; exhaustive handling |
| Railway programming | **CC-ERR** | Structured error propagation; no silent failures |
| Side effect isolation | **CC-TYP-03** (Encapsulation) | Pure core, impure shell; testable architecture |
...
2. Add immutability for domain objects (CC-FUN, CC-TYP)
   → Prevents mutation bugs in shared state

3. Use pipe/compose for data transformation (CC-FUN)
   → Cleaner than nested function calls

4. Add discriminated unions for state modeling (CC-TYP)
...
- [clean-code-standard.md](clean-code-standard.md) — CC-* rule definitions (CC-ERR, CC-FUN, CC-TYP)
