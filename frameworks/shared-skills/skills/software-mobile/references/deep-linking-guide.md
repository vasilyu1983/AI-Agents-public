...
Complete guide to deep linking across iOS and Android. Covers Universal Links, App Links, deferred deep linking, React Native and Expo integration, testing, routing architecture, and edge cases.
...
- [Apple App Site Association (AASA)](#apple-app-site-association-aasa)
- [AASA Requirements](#aasa-requirements)
- [Entitlements Configuration](#entitlements-configuration)
...
- [Getting SHA-256 Fingerprint](#getting-sha-256-fingerprint)
- [Debug keystore](#debug-keystore)
- [Release keystore](#release-keystore)
- [Google Play App Signing (from Play Console)](#google-play-app-signing-from-play-console)
- [Setup > App signing > SHA-256 certificate fingerprint](#setup-app-signing-sha-256-certificate-fingerprint)
- [AndroidManifest Intent Filters](#androidmanifest-intent-filters)
...
- [Attribution Without Third-Party SDK](#attribution-without-third-party-sdk)
- [React Native Deep Linking](#react-native-deep-linking)
- [React Navigation Linking Configuration](#react-navigation-linking-configuration)
- [Expo Linking](#expo-linking)
- [Short Links and QR Code Integration](#short-links-and-qr-code-integration)
- [Generating Branded Short Links](#generating-branded-short-links)
- [Deep Link Testing and Debugging](#deep-link-testing-and-debugging)
- [iOS Testing](#ios-testing)
- [Test Universal Link on simulator](#test-universal-link-on-simulator)
- [Test URI scheme on simulator](#test-uri-scheme-on-simulator)
- [Validate AASA file](#validate-aasa-file)
- [Check Apple CDN cache of your AASA](#check-apple-cdn-cache-of-your-aasa)
- [Android Testing](#android-testing)
- [Test App Link via adb](#test-app-link-via-adb)
- [Test URI scheme via adb](#test-uri-scheme-via-adb)
- [Verify App Links registration](#verify-app-links-registration)
- [Force re-verification](#force-re-verification)
- [Automated Deep Link Tests](#automated-deep-link-tests)
- [Routing Architecture for Deep Links](#routing-architecture-for-deep-links)
- [Centralised Deep Link Router](#centralised-deep-link-router)
- [Edge Cases](#edge-cases)
- [Cold Start vs Background vs Killed State](#cold-start-vs-background-vs-killed-state)
- [Common Universal Link Failures](#common-universal-link-failures)
- [Redirect Chain Problem](#redirect-chain-problem)
- [Smart App Banner (iOS Safari Fallback)](#smart-app-banner-ios-safari-fallback)
...
As of August 25, 2025, Firebase Dynamic Links is shut down. Use verified Universal Links / App Links plus approved attribution providers or a self-hosted post-install flow instead of planning new Firebase Dynamic Links integrations.
...
2. Server issues a short-lived invite or campaign token
3. User lands on a mobile web page and is sent to App Store / Play Store
4. On first launch, the app resumes through:
   - authenticated account lookup, or
   - explicit invite code / campaign token redemption
5. App fetches the original destination from the backend and routes to content

Limitations:
  - Less seamless than attribution-SDK solutions
  - Avoid fingerprinting-based matching; ATT and browser/app boundaries make it brittle
  - Requires explicit product design for post-install continuation
```
...
For new Expo apps, prefer Expo Router and let it own the linking configuration. Reach for `expo-linking` directly only when you need lower-level URL handling.
...
## Short Links and QR Code Integration
