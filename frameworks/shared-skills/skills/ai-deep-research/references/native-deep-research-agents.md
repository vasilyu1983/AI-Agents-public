...
|-----------|-------------------------------|------------------------------|-------------------------|-------------------------------------|
| **API surface** | Responses API; legacy aliases `o3-deep-research` / `o4-mini-deep-research` are marked deprecated in the model catalog. Their dated snapshots shut down 2026-07-23; the current deprecations table names `gpt-5.6-sol` as the replacement [verified: developers.openai.com API model catalog + deprecations, 2026-09-07]. The `o3-deep-research` model page still exists, so verify account-level availability before migration. | Interactions API (`POST /v1beta/interactions`); model IDs: `deep-research-preview-04-2026` / `deep-research-max-preview-04-2026` [verified: ai.google.dev, 2026-09-07] | Chat completions API; model ID: `sonar-deep-research` [verified: docs.perplexity.ai/api-reference/chat-completions-post] | Messages API; tool type: `web_search_20260318` (adds response-inclusion control) / `web_search_20260209` (dynamic filtering) / `web_search_20250305` (basic) — all GA, no beta header [verified: platform.claude.com/docs/en/docs/agents-and-tools/tool-use/web-search-tool, 2026-07-11] |
...
> **Deprecation notice (verify before building anything new)**: OpenAI's current model catalog marks `o3-deep-research` and `o4-mini-deep-research` deprecated. The deprecations page records that their dated `2025-06-26` snapshots shut down on **2026-07-23** and now names `gpt-5.6-sol` as the substitute. The standalone `o3-deep-research` model page still describes the legacy alias as available, so documentation surfaces remain inconsistent. Check account-level availability and re-evaluate tool access, source behavior, and report structure; `gpt-5.6-sol` is a general model and should not be assumed to reproduce the purpose-built workflow automatically. [Verified 2026-09-07 against the OpenAI model catalog, model page, and deprecations table.]

**Product status**: ChatGPT product access and API model access are separate. Verify the user's plan and API account; do not infer one from the other.

**Legacy o3 vs o4-mini tradeoff**: retain this only when replaying an existing capture or supporting an account where the aliases still resolve. Do not start a new integration around either deprecated alias.
...
**Limitations**:
- Deprecated model aliases with inconsistent documentation surfaces — confirm account availability and replacement behavior before any migration.
...
|----------|----------------|
| Need the deepest native synthesis | Compare currently available native research products on a representative query; do not route new work to deprecated OpenAI aliases |
| Need depth with lower cost | Compare current provider pricing and measured query cost; legacy `o4-mini-deep-research` is not a new-build default |
