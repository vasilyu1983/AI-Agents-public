...
**Last Updated:** 2026-07-11 (added validator client landscape / Firedancer status note)

---

## Table of Contents

1. [Validator Client Landscape](#validator-client-landscape)
2. [Anchor Framework Patterns](#anchor-framework-patterns)
3. [Account Security](#account-security)
4. [Program Derived Addresses (PDAs)](#program-derived-addresses-pdas)
5. [Cross-Program Invocations (CPIs)](#cross-program-invocations-cpis)
6. [SPL Token Integration](#spl-token-integration)
7. [Error Handling](#error-handling)
8. [Testing Strategies](#testing-strategies)
9. [Gas Optimization (Compute Units)](#gas-optimization-compute-units)
...
Solana runs on multiple validator client implementations, which matters for consensus resilience but is largely transparent to program (on-chain) development:

- **Agave** (formerly Solana Labs client) — the original, battle-tested validator client; still runs the majority of stake.
- **Firedancer** (Jump Crypto) — a from-scratch, high-performance validator client aimed at >1M TPS. It went live on mainnet in December 2025 and, as of mid-2026, runs on a meaningful and growing minority of active validators (client diversity, not majority adoption, is the current milestone) — do not assume Firedancer is the default or majority client. "Frankendancer" (Firedancer's networking/QUIC frontend paired with Agave's execution backend) has been the more common intermediate deployment while full Firedancer rollout continues.
- **Practical impact for developers**: client diversity reduces single-client bug/outage risk network-wide; it does not change Anchor/program-level APIs. Verify current validator-client market share (e.g., via Solana Compass or validators.app) before citing a specific adoption percentage — it is changing quickly.
...
- [SPL Programs](https://www.solana-program.com/docs)
- [Metaplex Standards](https://developers.metaplex.com/)
