...
## 4. SQL Server 2025 Query Optimization Features (verify current version)
...
```sql
-- OPPO requires compatibility level 170
SELECT name, compatibility_level
FROM sys.databases
WHERE name = DB_NAME();

-- Verify the database-scoped configuration
...
- Use the current OPPO documentation before changing compatibility level on an existing workload.
- Prefer Query Store evidence over guesswork when validating optional-parameter behavior.
...
```sql
-- Verify prerequisites commonly used with optimized locking guidance
SELECT
    name,
...
- Review the current optimized locking documentation before changing ADR or isolation defaults on a live workload
...
Use Query Store on readable secondaries when the workload and version support it:

- Performance history preserved during failovers
- Better tuning of read-only workloads

```sql
-- Verify Query Store state in the current database role
