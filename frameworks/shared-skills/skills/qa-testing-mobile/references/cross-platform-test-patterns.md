...
- [Shared Test Layer Architecture](#shared-test-layer-architecture)
- [Platform-Specific Test Isolation](#platform-specific-test-isolation)
- [Detox for React Native](#detox-for-react-native)
- [Flutter Integration Testing](#flutter-integration-testing)
- [Kotlin Multiplatform Test Sharing](#kotlin-multiplatform-test-sharing)
- [Appium Cross-Platform Patterns](#appium-cross-platform-patterns)
- [Test Data Sharing Strategies](#test-data-sharing-strategies)
- [CI Matrix Configuration](#ci-matrix-configuration)
- [When to Write Shared vs Platform-Specific Tests](#when-to-write-shared-vs-platform-specific-tests)
...
Use targeted searches instead of loading this whole file when possible:

- React Native / Detox: `rg -n "^## Detox|React Native" references/cross-platform-test-patterns.md`
- Flutter: `rg -n "^## Flutter" references/cross-platform-test-patterns.md`
- Kotlin Multiplatform: `rg -n "^## Kotlin Multiplatform|commonTest|iosTest|androidUnitTest" references/cross-platform-test-patterns.md`
- Appium: `rg -n "^## Appium|Page Object|driver" references/cross-platform-test-patterns.md`

Official docs to verify before final advice:

- React Native testing overview
- Flutter integration tests
- Kotlin Multiplatform test execution
- Appium 3 migration and compatibility guidance
...
React Native teams should keep most coverage in Jest/React Native Testing Library and reserve device E2E for critical flows. Use Detox when app-level synchronization is worth the extra wiring; use Maestro for additional black-box smoke coverage when breadth matters more than deep assertions.

**New Architecture (required as of React Native 0.82)**: The React Native New Architecture became the default in 0.76 (October 2024) and the only supported option in 0.82 (the old architecture option is ignored if set). Detox's own compatibility table (as verified 2026-07-11) lists official New Architecture support for RN 0.77.x through 0.84.x — newer RN releases may work but are not yet formally tested by the Detox maintainers. When testing RN 0.82+, confirm your installed Detox version's tested RN range before upgrading; compatibility updates can lag RN releases by a few weeks. A pragmatic staging pattern some teams use: enable the New Architecture in a development build (e.g. `expo-dev-client`) first, run the full Detox/Maestro + manual QA suite against it, and only flip the production build once the suite is green — this decouples "does our test suite support New Arch" from "is New Arch safe for users" as two separate gates.
...
module.exports = {
  ...
  devices: {
    simulator: { type: 'ios.simulator', device: { type: '<ios-simulator-name>' } },
    emulator: { type: 'android.emulator', device: { avdName: '<android-avd-name>' } },
  },
  configurations: {
...
Patrol 4.0 (released December 2025) added Flutter Web support, a VS Code extension with a working play button for `patrolTest()`, and improved debugging. As of 2026-07-11 the stable release is 4.6.x, and LeanCode has since shipped **Patrol MCP**, exposing `patrol develop` to AI coding agents (Claude Code, Cursor, etc.) for AI-assisted authoring, running, and debugging of Patrol E2E tests — the Flutter-ecosystem analogue of Maestro MCP. Patrol remains the recommended framework for Flutter E2E tests that need native automation (permissions, notifications, system dialogs). For projects using only `integration_test` without native interactions, the base Flutter SDK package is still sufficient.
...
```bash
# iOS simulator
flutter test integration_test/app_test.dart -d "<ios-simulator-name>"

# Android emulator
flutter test integration_test/app_test.dart -d emulator-5554

# All integration tests on both platforms (CI)
flutter test integration_test/ -d "<ios-simulator-name>"
