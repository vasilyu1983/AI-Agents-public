...
- [Frame Rate and Jank Detection](#frame-rate-and-jank-detection)
- [Memory Profiling](#memory-profiling)
- [Battery Drain Testing](#battery-drain-testing)
- [Network Performance](#network-performance)
- [App Size Optimization](#app-size-optimization)
- [Performance Budgets and CI Gates](#performance-budgets-and-ci-gates)
- [Automated Regression Detection](#automated-regression-detection)
...
Use network shaping to validate how the app behaves under realistic degraded conditions — slow 3G, high-latency LTE, and intermittent connectivity. This is distinct from simple offline testing: shaping controls bandwidth, latency, packet loss, and jitter simultaneously.

#### Linux / Android (tc netem)

`tc netem` (traffic control network emulator) is available on Linux CI hosts and inside Android's shell on rooted/emulator builds. Apply it on the host to throttle all traffic from the test runner to the backend.

```bash
# Add a 200ms delay with 20ms jitter and 1% packet loss on eth0
sudo tc qdisc add dev eth0 root netem delay 200ms 20ms loss 1%

# Change to simulate slow 3G (400ms delay, 50 kbps rate)
sudo tc qdisc change dev eth0 root netem delay 400ms rate 50kbit

# Remove shaping after the test run
sudo tc qdisc del dev eth0 root
```

Common profiles:

| Profile | Delay | Rate | Loss |
|---------|-------|------|------|
| 3G slow | 400ms | 50 kbps | 1% |
| 3G fast | 100ms | 1.6 Mbps | 0.5% |
| LTE degraded | 50ms | 10 Mbps | 0% |
| WiFi with jitter | 10ms ±5ms | 20 Mbps | 0% |
| Packet loss stress | 50ms | 5 Mbps | 5% |

On Android emulators, use the emulator's built-in `-netdelay` and `-netspeed` flags (see above) or `adb shell tc` on rooted images for finer control.

#### iOS (Network Link Conditioner)

The **Network Link Conditioner** (NLC) is built into iOS developer mode and macOS. On a simulator, enable it from the simulator's **Hardware → Network Link Conditioner** menu or set it programmatically via `simctl` environment variables.

```bash
# Enable a preset on a simulator via xcrun simctl (Xcode 15+)
# Preset names: "100% Loss", "3G", "DSL", "Edge", "High Latency DNS", "LTE", "Very Bad Network", "WiFi"
xcrun simctl spawn booted defaults write \
  com.apple.networklinkconditioner \
  profile -string "3G"

# Disable
xcrun simctl spawn booted defaults delete \
  com.apple.networklinkconditioner profile
```

For physical device testing, configure NLC in **Settings → Developer → Network Link Conditioner** (requires a developer profile). NLC presets are defined in `/Library/Preferences/com.apple.networklinkconditioner.plist` and can be inspected for exact delay/bandwidth values.

#### BrowserStack Network Profiles

BrowserStack App Automate exposes network profiles as a desired capability. Apply a profile to the entire test session; it shapes traffic between the device and the internet.

```json
{
  "browserstack.networkProfile": "2g-gprs-good",
  "browserstack.customNetwork": "1000,256,200,1"
}
```

`customNetwork` format: `download_kbps,upload_kbps,latency_ms,packet_loss_pct`.

Available named profiles include `no-network`, `2g-gprs-good`, `2g-gprs-lossy`, `3g-umts-good`, `3g-hsdpa`, `4g-lte`, `4g-lte-advanced-good`. Verify current profile names against BrowserStack docs before use.

#### Sauce Labs Network Throttling

Sauce Labs real-device cloud supports network conditions via the `saucelabs:options` capability or the Sauce UI during manual sessions.
...
Available conditions: `Offline`, `2G`, `3G`, `4G`, `LTE`. For custom profiles, use the Sauce REST API to set per-device network conditions mid-session.

#### Common Network-Shaping Test Cases

- **API timeout handling**: shape to 0 kbps and verify the app shows a recoverable error, not a crash.
- **Retry logic**: introduce 5% packet loss and confirm idempotent requests retry without duplicating side effects.
- **Image/media loading**: shape to 3G slow; verify lazy loaders show placeholders, not blank screens or layout shifts.
- **Offline → online transition**: cut network, perform an action, restore network; verify the app recovers and syncs pending changes.
- **Startup on poor network**: measure cold start time under 3G; ensure the app reaches an interactive state within the performance budget (even if content loads lazily).
