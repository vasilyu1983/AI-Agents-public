...
- [ ] Tested upgrade from previous production build where migration risk exists
- [ ] Tested on smallest supported screen size
- [ ] Tested on largest supported screen size
- [ ] Tested on foldable / large-screen posture if supported
...
- [ ] TestFlight build validated by internal/external beta path as planned
- [ ] Sign in with Apple compliance (if any third-party login)
- [ ] Widget extensions work if applicable
- [ ] Watch app syncs if applicable
- [ ] Accessibility audit reviewed for high-severity issues
...
- [ ] Google Play pre-launch report reviewed (if applicable to release path)
...
- [ ] Rollback thresholds defined for crashes, ANRs, and key-flow failures
