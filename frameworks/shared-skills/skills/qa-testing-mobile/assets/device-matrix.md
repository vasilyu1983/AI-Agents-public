...
| Android | Foldable / large-screen device | Latest Android | 2 | Posture, split-screen, and adaptive layouts |
| Android | Tablet (if supported) | Latest Android | 2 | Tablet layout testing |

## Coverage Dimensions To Add

- RAM/performance tier: include at least one constrained Android device if startup or scrolling performance matters.
- Form factor: include tablet and foldable coverage if layouts or multi-window behavior differ.
- Region/OEM mix: include market-dominant OEMs for your largest countries, not just global averages.
- Upgrade path: test the oldest supported OS and an app-upgrade path from the previous production build.
...
- Rebuild the matrix before major launches, new-country rollouts, or adaptive-layout work.
