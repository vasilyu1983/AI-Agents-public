...
for idx, layout in enumerate(prs.slide_master.slide_layouts):
    ...
    for ph in layout.placeholders:
        print(
            f"  Placeholder {ph.placeholder_format.idx}: "
...
def get_layout_by_name(prs: Presentation, wanted: str):
    for layout in prs.slide_master.slide_layouts:
        if layout.name == wanted:
            return layout
    available = [layout.name for layout in prs.slide_master.slide_layouts]
...
# Use branded layouts by name, not by position
title_slide = prs.slides.add_slide(get_layout_by_name(prs, 'Title Slide'))
title_slide.shapes.title.text = "Q4 Business Review"
title_slide.placeholders[1].text = "December 2025"

content_slide = prs.slides.add_slide(get_layout_by_name(prs, 'Title and Content'))
...
prs = Presentation('brand_template.pptx')
layout = next(
    layout for layout in prs.slide_master.slide_layouts
...
PPTX-Automizer is strongest when the template is designer-owned and the automation only replaces named elements while keeping the slide structure stable. Master/layout references usually survive well; animation timing and advanced media still require extra caution.
...
**Key advantage:** Designers maintain the template in PowerPoint. Engineers inject data with Automizer. Branding can stay pixel-perfect if template ids, placeholders, and media assumptions remain stable.
...
- Treat animation-heavy slides as test-required, not automatically safe
...
- Replacing animated or media-heavy shapes without retesting the generated deck
