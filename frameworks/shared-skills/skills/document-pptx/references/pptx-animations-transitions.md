...
|---|---|---|---|
| Slide transitions | No native API; XML manipulation required | Native `transition` option on slides | Can keep source-slide transitions when slides are copied unchanged |
...
| Timing / auto-advance | XML manipulation | `advanceAfter` option | Best-effort retention from the source slide |

**Key takeaway:** PptxGenJS is the cleanest path for transitions you control in code. For build animations, design them in PowerPoint first and keep post-generation edits minimal. `python-pptx` requires direct OOXML work, and `PPTX-Automizer` should be treated as best-effort preservation rather than a guaranteed animation-safe editing layer.
...
**Workflow:** Design animations in PowerPoint, save as template, use Automizer only for conservative text/image/table/chart replacement, then test the final file in desktop PowerPoint. Assume motion is best-effort, not guaranteed.
...
- Design animations in PowerPoint, then keep template edits conservative
...
- Assuming Automizer can safely rewrite animated shapes without side effects
