...
- Prefer OpenTelemetry-compatible instrumentation and OTLP export paths so local, hosted, and Aspire-managed environments share the same telemetry contract.
...
- If Aspire service defaults are used, are the inherited telemetry defaults still explicit enough for this service's operational needs?
