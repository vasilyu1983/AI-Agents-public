...
- EF Core is not fully Native AOT-compatible: reflection over the model/POCO graph produces trim warnings unless you commit to compiled models and source-generated configuration. If the host targets Native AOT, prefer Dapper/ADO.NET for that data path (see `references/aspnet-core-api-patterns.md`).
