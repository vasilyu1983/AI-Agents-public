...
{
    public static IServiceCollection AddOrdersApiClient(this IServiceCollection services, IConfiguration configuration)
    {
        var ordersApiOptions =
            configuration.GetSection(OrdersApiOptions.SectionName).Get<OrdersApiOptions>()
            ?? throw new InvalidOperationException("OrdersApi configuration section is required.");

        services
            .AddOptions<OrdersApiOptions>()
            .Bind(configuration.GetSection(OrdersApiOptions.SectionName))
            .Validate(options => Uri.TryCreate(options.BaseUrl, UriKind.Absolute, out _), "OrdersApi:BaseUrl must be an absolute URI.")
            .Validate(options => options.AttemptTimeoutSeconds > 0, "OrdersApi:AttemptTimeoutSeconds must be greater than zero.")
            .Validate(options => options.TotalRequestTimeoutSeconds >= options.AttemptTimeoutSeconds, "OrdersApi:TotalRequestTimeoutSeconds must be >= AttemptTimeoutSeconds.")
            .ValidateOnStart();
        ...
        services
            ...
            .AddStandardResilienceHandler(options =>
            {
                ...
                options.AttemptTimeout.Timeout = TimeSpan.FromSeconds(ordersApiOptions.AttemptTimeoutSeconds);
                options.TotalRequestTimeout.Timeout = TimeSpan.FromSeconds(ordersApiOptions.TotalRequestTimeoutSeconds);
            });
