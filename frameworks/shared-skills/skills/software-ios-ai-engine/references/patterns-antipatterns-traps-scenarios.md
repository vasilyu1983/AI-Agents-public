...
- **P16 · Prompt budget against runtime context capacity.** Read `contextSize`; token-count instructions, prompt, tool definitions, tool outputs, `@Generable` schemas, and expected response with `tokenCount(for:)`. Drop the lowest-confidence chunks first. The A-prompt is never "dump the whole bundle and hope."
...
| T6a · Tool/schema overhead omitted from token budget | "Short prompt" still exceeds the model's capacity | Count instructions + prompt + tools + schemas + response; query `contextSize`/`tokenCount(for:)` instead of a hardcoded constant |
