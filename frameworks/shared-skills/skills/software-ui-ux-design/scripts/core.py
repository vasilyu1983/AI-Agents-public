...
CSV_CONFIG = {
    ...
    "motion": {
        "file": "motion.csv",
        "search_cols": ["Category", "Intensity Tier", "Keywords", "Trigger", "Framework Notes"],
        "output_cols": ["Category", "Intensity Tier", "Keywords", "Trigger", "Duration", "Easing", "GSAP Snippet", "Framework Notes", "Do", "Don't", "Performance Notes"]
    },
    "ui-reasoning": {
        "file": "ui-reasoning.csv",
        "search_cols": ["UI_Category", "Recommended_Pattern", "Style_Priority", "Color_Mood", "Typography_Mood"],
...
# ============ SLIDE DECK GROUP ============
# data/slides/ - slide/presentation design guidance. Each file has its own
# column shape (unlike stacks, which share one shape), so columns are
# spelled out per entry rather than reused from a common dict.
SLIDE_CONFIG = {
    "backgrounds": {
        "file": "slides/slide-backgrounds.csv",
    ...
    "charts": {
        ...
        "search_cols": ["chart_type", "keywords", "best_for", "data_type", "when_to_use"],
        "output_cols": ["chart_type", "keywords", "best_for", "data_type", "when_to_use", "when_to_avoid", "max_categories", "slide_context", "css_implementation", "accessibility_notes", "sources"]
    },
    "color-logic": {
        "file": "slides/slide-color-logic.csv",
        "search_cols": ["emotion", "background", "accent_usage"],
    ...
    "layout-logic": {
        "file": "slides/slide-layout-logic.csv",
        "search_cols": ["goal", "emotion", "layout_pattern", "direction"],
        "output_cols": ["goal", "emotion", "layout_pattern", "direction", "visual_weight", "break_pattern", "use_bg_image"]
    },
    "layouts": {
        "file": "slides/slide-layouts.csv",
        "search_cols": ["layout_name", "keywords", "use_case", "recommended_for", "avoid_for"],
        "output_cols": ["layout_name", "keywords", "use_case", "content_zones", "visual_weight", "cta_placement", "recommended_for", "avoid_for", "css_structure", "card_variant", "metric_style", "quote_style", "grid_columns", "visual_treatment", "animation_class"]
    },
    "strategies": {
        "file": "slides/slide-strategies.csv",
        "search_cols": ["strategy_name", "keywords", "goal", "audience", "tone", "narrative_arc"],
        "output_cols": ["strategy_name", "keywords", "slide_count", "structure", "goal", "audience", "tone", "narrative_arc", "key_metrics", "sources", "emotion_arc", "sparkline_beats"]
    },
    "typography": {
        "file": "slides/slide-typography.csv",
...
AVAILABLE_SLIDES = list(SLIDE_CONFIG.keys())

# ============ CIP (CORPORATE IDENTITY / BRAND) GROUP ============
# data/cip/ - corporate identity program guidance (industry brand direction,
# deliverable specs, mockup-scene prompts). Per-file column shapes, same
# reasoning as SLIDE_CONFIG above.
CIP_CONFIG = {
    "industries": {
        "file": "cip/industries.csv",
    ...
    "deliverables": {
        "file": "cip/deliverables.csv",
        "search_cols": ["Deliverable", "Category", "Keywords", "Description"],
    ...
    "mockup-contexts": {
        "file": "cip/mockup-contexts.csv",
        "search_cols": ["Context Name", "Category", "Keywords", "Scene Description", "Best For"],
...
AVAILABLE_CIPS = list(CIP_CONFIG.keys())


def _drift_check():
    """Filesystem-driven drift check: every CSV under data/ must be reachable
    via CSV_CONFIG, STACK_CONFIG, SLIDE_CONFIG, or CIP_CONFIG. Returns a list
    of on-disk CSV paths (relative to DATA_DIR) with no matching config entry.
    Call this from tests/CI, not on the hot search path.
    """
    configured_files = set()
    for cfg in (CSV_CONFIG, STACK_CONFIG, SLIDE_CONFIG, CIP_CONFIG):
        for entry in cfg.values():
            configured_files.add(entry["file"])
    ...
    return sorted(on_disk - configured_files)
...
def search(query, domain=None, max_results=MAX_RESULTS):
    ...
    if domain not in CSV_CONFIG:
        valid = ", ".join(sorted(CSV_CONFIG.keys()))
...
def search_slide(query, slide, max_results=MAX_RESULTS):
    """Search slide/presentation-deck guidance (data/slides/)"""
    if slide not in SLIDE_CONFIG:
        return {"error": f"Unknown slide domain: {slide}. Available: {', '.join(AVAILABLE_SLIDES)}"}

    config = SLIDE_CONFIG[slide]
    filepath = DATA_DIR / config["file"]

    if not filepath.exists():
        return {"error": f"Slide file not found: {filepath}", "slide": slide}
...
def search_cip(query, cip, max_results=MAX_RESULTS):
    """Search corporate identity program guidance (data/cip/)"""
    if cip not in CIP_CONFIG:
        return {"error": f"Unknown CIP domain: {cip}. Available: {', '.join(AVAILABLE_CIPS)}"}

    config = CIP_CONFIG[cip]
    filepath = DATA_DIR / config["file"]

    if not filepath.exists():
        return {"error": f"CIP file not found: {filepath}", "cip": cip}
