...
- `/subtask <directive>` — the current on-demand forked-subagent command on v2.1.212 or later. With agent view disabled, `/subtask` is unavailable and `/fork` starts the forked subagent; with agent view enabled, `/fork` creates a separate background session. On v2.1.161 through v2.1.211, `/fork` was the forked-subagent command.
- `CLAUDE_CODE_FORK_SUBAGENT` — env var honored in interactive mode, `claude -p`, and the Agent SDK. Set to `1` to enable fork mode explicitly (including allowing the model to request the `fork` subagent type), or `0` to disable it everywhere, including staged rollout. Omitting an agent type still selects `general-purpose`; a model-spawned fork must request `fork` explicitly. Claude-initiated forking remains experimental.
...
Because a Claude Code conversation fork's system prompt and tool definitions are identical to the parent's at spawn time, its first request reuses the parent's prompt-cache prefix. That establishes cache lineage, not lower total task cost: inspect cache reads and writes, uncached input, output, tools, retries, and outcome before comparing it with a named subagent. Tool calls remain isolated; only the final result returns to the parent's conversation, though the fork can also be given `isolation: "worktree"` so its file edits land in a separate git worktree instead of the parent's checkout. See [`../../ai-coding-agents-sessions/references/context-forking.md`](../../ai-coding-agents-sessions/references/context-forking.md) for session-lifecycle implications and `agents-subagents` §"Forking Parent Context Into Subagents" for design-side rules.
