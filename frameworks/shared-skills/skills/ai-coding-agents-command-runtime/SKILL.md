...
7. **Treat forked commands as subagent orchestration.** Give them explicit allowed tools, agent selection, and a runtime-scoped context-start mode. In Claude Code, a documented conversation fork inherits parent history and shares its prompt-cache prefix; a command named “fork” in another runtime proves neither behavior.
