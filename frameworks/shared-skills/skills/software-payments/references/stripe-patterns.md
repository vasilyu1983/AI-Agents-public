...
- [Smart Retry Configuration (Stripe Dashboard)](#smart-retry-configuration-stripe-dashboard)
...
- [Stripe API Version Notes (2025-2026)](#stripe-api-version-notes-2025-2026)
...
This is an emerging pattern for hybrid subscription + usage billing.
...
- [ ] PCI scope confirmed: hosted Checkout/Payment Links → SAQ A; Payment Element → SAQ A-EP; custom form → SAQ D. Choose the lowest-scope surface your UX requirements allow.
- [ ] No raw card data (number, CVV, expiry) logged, stored, or passed through your servers at any point.
- [ ] Compliance scope assumes PCI DSS v4.0.1 (the sole active version) with all future-dated requirements mandatory since 31 March 2025 — do not scope against v4.0's original 2022 baseline. Route full-standard compliance work to a dedicated security/compliance review; this checklist covers the checkout-surface scope-reduction choice, not the full assessment.
...
| Version | Key Changes |
|---------|-------------|
| `2025-06-30.basil+` | Flexible `billing_mode`; required for mixed-interval subscriptions and newer billing capabilities |
...
| `2026` | Managed Payments reached general availability for eligible digital-product sellers; rollout remains eligibility-gated (verify current scope before committing) |
