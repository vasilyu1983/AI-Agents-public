...
- Use `--artifacts-path` or one stable NUKE artifacts root when ad hoc CLI runs would otherwise collide.
...
- Port collision in Docker-backed suites: switch to dynamic host-port reservation and verify tests still pass when old fixed ports are intentionally occupied.
- Container endpoint mismatch (works locally, fails in CI): resolve the advertised Docker host from Testcontainers API or `DOCKER_HOST` instead of hard-coding `127.0.0.1`.
