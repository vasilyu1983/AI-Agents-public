...
- Confirm the repository is not mixing VSTest and `Microsoft.Testing.Platform` expectations in one command path.
...
- Verify digest extraction is coming from a structured output when available, or that log parsing is still valid.
