...
Use this when the user needs artifact lineage, promotion policy, packaging standards, or rollback-ready model metadata.

## Table of Contents

- [Quick Navigation](#quick-navigation)
- [Overview](#overview)
- [Pattern 1: Model Registry Structure](#pattern-1-model-registry-structure)
- [Core Components](#core-components)
- [Example Registry Entry](#example-registry-entry)
- [Pattern 2: Model Packaging](#pattern-2-model-packaging)
- [Packaging Goals](#packaging-goals)
- [Packaging Strategies](#packaging-strategies)
- [Install system dependencies](#install-system-dependencies)
- [Copy model artifact and dependencies](#copy-model-artifact-and-dependencies)
- [Copy inference code](#copy-inference-code)
- [Set entrypoint](#set-entrypoint)
- [environment.yml](#environmentyml)
- [Preprocessing and Postprocessing Code](#preprocessing-and-postprocessing-code)
- [preprocessing.py (embedded with model)](#preprocessingpy-embedded-with-model)
- [Pattern 3: Promotion Flow](#pattern-3-promotion-flow)
- [Stage Progression](#stage-progression)
- [Stage Definitions](#stage-definitions)
- [Promotion Criteria](#promotion-criteria)
- [Promotion Workflow](#promotion-workflow)
- [Pattern 4: Versioning Strategies](#pattern-4-versioning-strategies)
- [Semantic Versioning](#semantic-versioning)
- [Auto-Incremented Versioning](#auto-incremented-versioning)
- [Git Commit-Based Versioning](#git-commit-based-versioning)
- [Pattern 5: Lineage Tracking](#pattern-5-lineage-tracking)
- [What to Track](#what-to-track)
- [Example Lineage Graph](#example-lineage-graph)
- [Checklist](#checklist)
- [Pattern 6: Governance and Compliance](#pattern-6-governance-and-compliance)
- [Model Registry as Audit Trail](#model-registry-as-audit-trail)
- [Compliance Features](#compliance-features)
- [Pattern 7: Multi-Registry Strategy](#pattern-7-multi-registry-strategy)
- [When to Use Multiple Registries](#when-to-use-multiple-registries)
- [Synchronization Strategies](#synchronization-strategies)
- [Tools & Frameworks](#tools-&-frameworks)
- [Real-World Example: Recommendation Model Registry](#real-world-example-recommendation-model-registry)
- [Context](#context)
...
MLflow 3 and most modern registries favour **aliases** over hard lifecycle stages. Stages are soft-deprecated in MLflow 3 — use aliases such as `@champion` and `@challenger` for promotion governance. Other registries (W&B, ZenML) follow similar alias or tag patterns. Verify current MLflow docs at https://mlflow.org/docs/latest/ before advising.

Common alias conventions:
- `@candidate`: Validated on hold-out data, awaiting shadow/canary
- `@shadow`: Running alongside champion for comparison
- `@challenger`: Deployed to canary receiving limited live traffic
- `@champion`: Serving the majority of production traffic
- `@archived`: Retired, retained for compliance or rollback
...
### Promotion Progression (Alias-Based — MLflow 3 / modern registries)

```
(untagged) → @candidate → @challenger → @champion → @archived
```

> **MLflow 3 note:** Lifecycle stages (`Staging`, `Production`) are soft-deprecated. Prefer aliases for all promotion governance. Verify at https://mlflow.org/docs/latest/ before advising.

### Alias Definitions

**@candidate:**
- Validated on hold-out data; metrics meet acceptance criteria
- Passed regression-eval CI gate; ready for shadow/canary

**@challenger:**
- Deployed to canary; receiving limited live traffic (5–20%)
- Champion/challenger comparison running

**@champion:**
- Serving the majority of production traffic
- Continuously monitored; SLAs enforced
...
> **Note:** Neptune.ai hosted service shut down on 2026-03-05 (acquired by OpenAI). Do not recommend Neptune.ai as a live hosted service.
