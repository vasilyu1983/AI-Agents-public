...
Use this when the user needs production eval loops, shadow traffic, canaries, online experiments, or human feedback collection.

## Table of Contents

- [Quick Navigation](#quick-navigation)
- [Overview](#overview)
- [Pattern 1: Feedback Signal Collection](#pattern-1-feedback-signal-collection)
- [Types of Signals](#types-of-signals)
- [Implementation: Capture Signals with Privacy](#implementation-capture-signals-with-privacy)
- [Privacy Considerations](#privacy-considerations)
- [Checklist](#checklist)
- [Pattern 2: Shadow & Canary Deployments](#pattern-2-shadow-&-canary-deployments)
- [Shadow Deployment](#shadow-deployment)
- [Canary Deployment](#canary-deployment)
- [Checklist](#checklist)
- [Pattern 3: A/B Testing & Statistical Significance](#pattern-3-ab-testing-&-statistical-significance)
- [A/B Test Design](#ab-test-design)
- [Implementation](#implementation)
- [Checklist](#checklist)
- [Pattern 4: Human-in-the-Loop Labeling](#pattern-4-human-in-the-loop-labeling)
- [Use Cases](#use-cases)
- [Labeling Pipeline](#labeling-pipeline)
- [Checklist](#checklist)
- [Pattern 5: Retraining Cadence & Promotion](#pattern-5-retraining-cadence-&-promotion)
- [Retraining Triggers](#retraining-triggers)
- [Retraining Pipeline](#retraining-pipeline)
- [Checklist](#checklist)
- [Real-World Example: Recommendation Model Feedback Loop](#real-world-example-recommendation-model-feedback-loop)
- [Context](#context)
- [Feedback Collection](#feedback-collection)
- [User clicks on recommendation](#user-clicks-on-recommendation)
- [User purchases item](#user-purchases-item)
- [Compute daily metrics](#compute-daily-metrics)
- [Retraining Pipeline](#retraining-pipeline)
- [A/B Test](#ab-test)
- [Outcome](#outcome)
...
## LLMOps Observability: Langfuse vs Braintrust

When choosing an LLMOps platform for production traces and evals, the two most common alternatives to LangSmith are:

| Dimension | Langfuse | Braintrust |
|-----------|----------|------------|
| License | MIT (self-hosted); managed cloud available | Proprietary (cloud-only) |
| Primary strength | Observability-first: traces, prompt management, cost tracking, dataset curation | Eval-first: CI/CD gating, production traces that automatically become eval cases |
| OTel integration | Native OpenTelemetry support | OTel-compatible; own SDK preferred |
| Self-hosting | Yes — standard production path | No — cloud-only |
| Eval workflow | Evals available; observability is the center of gravity | Evaluation is the center of gravity; CI/CD blocking is a first-class feature |
| Production traces to evals | Manual curation from traces | Automatic conversion of production traces into eval cases |

**When to pick Langfuse:** primary need is production observability, self-hosted data residency, OTel-native instrumentation, and tracing. Evals are supported but secondary.

**When to pick Braintrust:** primary need is eval-first CI/CD gating, with production traces automatically surfaced as eval candidates. Requires accepting proprietary cloud.

Both sit alongside ArizeAI and AgentOps in the observability landscape. The right choice depends on whether your team's workflow centers on traces (Langfuse) or evaluation gates (Braintrust).
...
## Pattern 6: Eval-as-CI-Gate (Canonical Pattern)

**This skill is the canonical owner of this pattern.** Cross-links from other skills point here for full implementation detail.

A regression-eval suite that blocks model or prompt deployment in CI/CD unless quality thresholds are met.

### What It Is

- A curated test suite of representative inputs + expected outputs (or quality scores)
- Run automatically on every model or prompt PR/deployment candidate
- CI gate: if the suite regresses beyond a threshold vs. the current champion, the deployment is blocked

### Structure

```yaml
# eval-gate.yaml — example CI gate configuration
eval_suite:
  dataset: "s3://evals/regression-suite-v3.jsonl"   # versioned gold set
  judge: llm-as-judge                                 # or heuristic / human
  metrics:
    - name: task_success_rate
      threshold: 0.85          # block if drops below this
      regression_limit: 0.03   # block if regresses >3% vs champion
    - name: safety_refusal_rate
      threshold: 0.99
      regression_limit: 0.01
  champion_baseline: "@champion"   # compare against current champion alias
```

### LLM-as-Judge Calibration Caveat

LLM-as-judge scoring is convenient but **judges drift** as model versions change:
- A judge model upgrade can shift scores without any change in the evaluated model
- Calibrate the judge against a human-labeled gold set before relying on it for CI gates
- Re-calibrate whenever the judge model is updated or when scores shift unexpectedly
- Record judge model version alongside every eval run for auditability

```python
def run_eval_gate(candidates, suite_path, judge_model, champion_scores):
    """
    Run eval suite against candidates; block if regression detected.
    Returns: (passed: bool, report: dict)
    """
    results = []
    for item in load_suite(suite_path):
        response = generate(item['input'], model=candidates['model'])
        score = judge(response, item['reference'], model=judge_model)
        results.append({'id': item['id'], 'score': score})

    candidate_mean = mean(r['score'] for r in results)
    champion_mean = champion_scores['task_success_rate']
    regression = champion_mean - candidate_mean

    passed = (
        candidate_mean >= 0.85 and
        regression <= 0.03
    )
    return passed, {
        'candidate_score': candidate_mean,
        'champion_score': champion_mean,
        'regression': regression,
        'judge_model': judge_model,   # log for auditability
        'suite_version': suite_path,
    }
```

### Checklist

- [ ] Eval suite dataset versioned and protected from contamination
- [ ] Thresholds and regression limits defined per metric
- [ ] Judge model version logged alongside every eval run
- [ ] Judge calibrated against human-labeled gold set
- [ ] CI gate blocks deployment on regression; reports diff vs champion
- [ ] Suite reviewed and updated on every major domain shift
