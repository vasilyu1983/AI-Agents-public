...
**Freshness note:** Vendor versions and orchestration capabilities are time-sensitive. Verify current docs before recommending a specific stack.

Use this when the user needs a controlled retraining loop with explicit triggers, gates, and rollback criteria.

## Table of Contents

- [Quick Navigation](#quick-navigation)
- [Decision Tree: Retraining Trigger Selection](#decision-tree-retraining-trigger-selection)
- [Quick Reference: Retraining Strategies](#quick-reference-retraining-strategies)
- [Judgment: When Not to Retrain](#judgment-when-not-to-retrain)
- [Operational Patterns](#operational-patterns)
- [Pattern 1: Drift Detection Triggers](#pattern-1-drift-detection-triggers)
- [Pattern 2: Data Preparation Pipeline](#pattern-2-data-preparation-pipeline)
- [Pattern 3: Training Orchestration (Airflow)](#pattern-3-training-orchestration-airflow)
- [Pattern 4: Champion/Challenger Validation Gates](#pattern-4-championchallenger-validation-gates)
- [Pattern 5: Deployment and Rollback](#pattern-5-deployment-and-rollback)
- [Pattern 6: Post-Deployment Monitoring](#pattern-6-post-deployment-monitoring)
...
## Judgment: When Not to Retrain

A drift alert or a scheduled trigger firing is an invitation to investigate, not an automatic mandate to retrain. Retraining has real costs — compute spend, validation time, the risk of introducing a regression a stale-but-known-good model doesn't have — and an automated pipeline that retrains reflexively on every trigger is often worse than one gated by a human decision point. Judgment calls that separate a mature retraining program from a reflexive one:

- **Confirm the drift is real and consequential before spending a training run on it.** Cross-check the drift signal against a downstream performance proxy (delayed-label accuracy, business KPI) or at minimum a second detection window. A retraining pipeline that fires on every PSI blip retrains on noise and burns the team's trust in the automation the first time it promotes a worse model.
- **Ask whether the fix is upstream, not in the model.** Drift caused by a broken feature pipeline, a schema change, or a stale join key is a data-engineering bug — retraining on corrupted data teaches the model the bug. Triage upstream health (feature store freshness, pipeline logs, schema diffs) before queuing a retrain; see [Drift Detection Guide](drift-detection-guide.md) for the triage step.
- **Weigh retraining cost against the cost of running one more cycle on the current model.** A model that has degraded by 1-2% PR-AUC in a low-stakes recommendation context may not be worth an emergency retrain-and-promote cycle; the same delta in a fraud or credit model usually is. Set the performance-drop threshold per model based on the cost of a false negative/positive in that domain, not a single global default.
- **Don't let "we can retrain" substitute for "we should retrain."** Cheap, automated retraining removes the friction that used to force a conversation about *why* performance degraded. Keep a lightweight review step (even just a Slack post with drift summary + decision) so degradation patterns get investigated, not just papered over by a fresh model on the same broken assumption.
- **Respect the cooldown even under pressure.** When a stakeholder asks for an emergency retrain outside the cooldown window, that's a signal the cooldown or trigger sensitivity needs revisiting — not a reason to bypass it ad hoc. Ad hoc bypasses are how retraining pipelines end up retraining hourly during an incident and destabilizing an otherwise-fine model.
- **Regulated or safety-critical models: default to scheduled + reviewed, not adaptive.** Continuous or drift-triggered retraining without a mandatory human review gate is very hard to defend to an auditor asking "why did the model change on this date." If the domain requires explainable change control, prefer scheduled retraining with a documented approval step over adaptive triggers, even if it means tolerating known drift a few weeks longer.
...
def promote_to_production(ti):
    """Blue-green deployment with automatic rollback (alias-based, MLflow 3)."""
    ...
    # Step 2: Tag as challenger, deploy to canary (10% traffic)
    ...
    if canary_metrics['error_rate'] > 0.01 or canary_metrics['latency_p99'] > 150:
        # Rollback canary; remove challenger alias
        rollback_canary()
        client.delete_registered_model_alias("fraud-model", "challenger")
        raise ValueError(f"Canary failed: {canary_metrics}")

    # Step 4: Ramp to 100%; promote to champion
    deploy_full(model_version=mv.version)
    client.set_registered_model_alias("fraud-model", "champion", mv.version)
    client.delete_registered_model_alias("fraud-model", "challenger")

def rollback_to_previous():
    """Emergency rollback: redeploy previous champion by version number."""
    client = MlflowClient()
    # Identify previous version by searching registry history
    # (aliases don't stack — store the previous champion version in a tag or CI artifact)
    previous_version = get_previous_champion_version("fraud-model")  # custom helper

    # Redeploy previous version
    deploy_full(model_version=previous_version)
    client.set_registered_model_alias("fraud-model", "champion", previous_version)

    # Alert team
    send_alert("Model rollback executed", severity="high")

# NOTE: MLflow 3 aliases do not stack. Track the previous champion version number
# in a CI artifact or registry tag so rollback can target it precisely.
```
...
## Pattern 7: Training Job Orchestration and Cost Guard

- **Use when:** Running fine-tune or large retraining jobs that need queuing, scheduling, and budget protection

### Queue/Scheduler Options

| Orchestrator | Best For | Cost Control |
|---|---|---|
| Airflow / Dagster / Prefect | General ML pipelines, DAG-based scheduling | Budget sensors, SLA miss alerts |
| Ray (Ray Train + Ray Tune) | Distributed training, hyperparameter sweeps | Resource limits per job |
| AWS SageMaker Training Jobs | Managed, per-job spot pricing | Spot instance support, budget alerts |
| Vertex AI Training (Google) | Managed, TPU/GPU queues | Budget alerts, quotas |
| Slurm | HPC clusters, on-prem GPU farms | Partition limits, fairshare scheduling |

### Key Practices

```python
# Budget guard: abort if cost estimate exceeds cap before launching
def launch_training_job_with_guard(config, max_budget_usd=500):
    """Queue training job only if estimated cost is within budget."""
    ...
    if estimated_cost > max_budget_usd:
        raise ValueError(
            f"Training job aborted: estimated ${estimated_cost:.0f} exceeds "
            f"budget cap of ${max_budget_usd}. Reduce GPU count or time."
        )
    return submit_job(config)

# Cooldown guard: prevent queue storms from drift-triggered retraining
RETRAINING_COOLDOWN_HOURS = 24

def should_retrain(last_retrain_timestamp):
    """Enforce cooldown between retraining runs to prevent runaway costs."""
    hours_since = (datetime.utcnow() - last_retrain_timestamp).total_seconds() / 3600
    return hours_since >= RETRAINING_COOLDOWN_HOURS
```

### Chargeback / Showback Per Team or Per Model

Tag every training job and inference workload with team, model, and environment tags. Route costs to a cost dashboard (Grafana, cloud billing console, or DataDog) broken down by:
- **Team** — which team's budget is consumed
- **Model** — cost per model version (training + serving)
- **Environment** — dev/staging/production split (catch dev waste)

See [Cost Management & FinOps](cost-management-finops.md) for implementation details.
