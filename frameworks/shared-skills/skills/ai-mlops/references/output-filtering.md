...
Use this when the system needs a final safety gate before responses reach users or downstream systems.
...
- [Layer 1: Classifier Gate](#layer-1-classifier-gate)
- [Layer 2: Rule-Based Filtering](#layer-2-rule-based-filtering)
- [Layer 3: LLM-Based Post-Filter or Rewriter](#layer-3-llm-based-post-filter-or-rewriter)
- [Layer 4: Scoring & Thresholds](#layer-4-scoring-&-thresholds)
- [Output Filtering Checklist](#output-filtering-checklist)
- [Real-World Example: Multi-Layer Output Filter](#real-world-example-multi-layer-output-filter)
...
class LLMPostFilter:
    def __init__(self):
        # Use a lightweight model for cost efficiency — inject from config, do not hardcode
        self.filter_llm = LLM(model=config['llm']['model_filter'])
